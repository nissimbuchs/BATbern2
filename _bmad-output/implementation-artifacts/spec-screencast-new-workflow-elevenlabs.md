...
- Promote-created users are named `USER_FIRST_NAME`/`USER_LAST_NAME` (Bruno/Test) from `e2e/helpers/test-data-factory.ts` — CUMS derives the username from `firstname.lastname`, so the names (not the email) are what makes the `bruno.test%` global-teardown sweep remove the CUMS rows. `email()` supplies the never-deliverable address. Cognito accounts are NOT removed by any sweep (CUMS delete is DB-only) — documented manual cleanup. The kanban card keeps the brainstorm name visible as a caption, so name-based card lookup still works post-promote. [Renegotiated with Nissim 2026-06-07 — original clause wrongly claimed email() drives the sweep.]
