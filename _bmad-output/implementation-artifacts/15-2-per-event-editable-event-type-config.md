...
- 2026-06-21 — Manual-test fixes (Nissim): the APERITIF session type was only handled in the slot-assignment grid; added `'aperitif'` as a structural type to all other agenda surfaces (organizer `SpeakersSessionsTable` agenda table, public `EventProgram`/`SessionCards`/`SpeakerGrid`/`EventCard`, `SessionEditModal`, live-control `AgendaList`, presenter `usePresentationSections`/`AgendaView`) so it renders as a structural block (like break/lunch), not an unassigned speaker slot. Also fixed `HomePage` `public.registrationClosed` (wrong namespace → `tCommon`). Note: the public homepage shows the agenda timeline only when `currentPublishedPhase === AGENDA` (pre-existing phase logic) — a LIVE event with phase SPEAKERS shows session cards by design; not changed.
