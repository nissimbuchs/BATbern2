...
| 2026-06-12 | Abuse-surface review (PO): (1) **Notes stay open to anyone** — confirmed acceptable given the existing 500-char `@Size` cap + Turnstile + 5/IP/hour rate limit (no backend change). (2) **Soft one-per-browser guard for anonymous** repeat-thanks added in `ThankOrganizersNavButton`: on a successful anonymous thank we persist `localStorage['batbern.thanked.<eventCode>']` and, on popover close, swap the trigger for a disabled "You thanked" pill (`thanks-nav-thanked`); the server rate limit remains the hard backstop. Logged-in users are unaffected (server-deduped to 1/event, note still editable). New i18n key `thanks.widget.alreadyThanked` ×10 locales. +2 FE tests (7 total); verified in-browser (pill renders disabled, persists on reload, absent for logged-in). |
| 2026-06-12 | UX redesign (PO request): replaced the inline card entirely with a compact "like"-style button in the public **top navigation** (`ThankOrganizersNavButton`, Tailwind + Radix Popover, no MUI) — a 🙏/♥ + live count; click opens a small note popover that dismisses on outside-click (native Radix behavior). Event context reaches the nav via a new `thankableEventCode` prop threaded `HomePage → PublicLayout → PublicNavigation` (set only when `workflowState IN EVENT_LIVE/EVENT_COMPLETED`); the nav stays event-agnostic. Shown on desktop CTA area + mobile menu. Invisible Turnstile widget kept always-mounted (outside the popover) so `useTurnstile`'s render effect attaches and the token still generates in prod. Old `ThankOrganizersWidget` + test deleted; new `ThankOrganizersNavButton.test.tsx` (5 tests). Reused all `thanks.widget.*` i18n keys (no new keys). FE tests green; tsc/ESLint clean; verified in-browser. |
