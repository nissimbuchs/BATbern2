...
> **Spec amendment (2026-05-17, post-review)** — the affirmative paths for #31 (filter-on-click) and #32 (red chip) require backdating `updated_at` on seeded speakers, which the public API does NOT expose. Per code-review Decision D1 (PM-ratified), the Playwright spec asserts only the **wire-up + negative-case regression** for #31 and #32; the affirmative warning/error transitions are fully covered at the Vitest level (`kanbanThresholds.test.ts` cases #4-7 + boundary tests, `SpeakerStatusLanes.test.tsx` cases #21-30). If a future story extends the seed helpers with a `created_at` / `updated_at` override, re-promote #31 + #32 to affirmative Playwright tests.

31. `it('should not render CONTACTED sub-line when no cards are stale (negative-case regression)', ...)` — seed a single fresh CONTACTED card. Assert the `status-lane-subline-contacted` element has count 0. The affirmative click-to-filter behaviour is covered by Vitest case #29.
32. `it('should render time-in-state chip with data-severity attribute (Story 11.D.3 wire-up)', ...)` — seed a fresh CONTACTED card. Assert the chip has `data-severity="normal"` and the MUI `MuiChip-colorDefault` class. The affirmative `data-severity="error"` + `MuiChip-colorError` transitions live in Vitest cases #21-28.
33. `it('should display "Slot capacity reached" in the READY column as static (non-clickable) text', ...)` — seed: 2 ACCEPTED, 1 READY. If the event-type default `maxSlots <= 2` the gate fires and the test asserts the sub-line text + non-`<button>` tag; otherwise the test annotates a skip (the deterministic gate-fired case is Vitest case #23).
...
Adversarial review on 2026-05-17 (bmad-code-review, 3-layer: Blind Hunter + Edge Case Hunter + Acceptance Auditor, all Opus). 60 raw findings → 33 after dedup → 3 decision-needed (all resolved), 22 patches + 3 from decisions = **25 patches applied**, 8 deferred, 14 dismissed.

#### Decision-needed — resolved

- [x] [Review][Decision] **AC5 Playwright #31 + #32 substituted negative cases for the spec's affirmative cases** — Resolved: ratified Vitest-only affirmative coverage. AC5 amended in-spec (above) to authorise the negative/wire-up Playwright cases the dev shipped; if future seed helpers expose `updated_at` override, re-promote to affirmative. (source: auditor)
- [x] [Review][Decision] **INVITED at exactly the deadline classifies as `warning`, not `error`** — Resolved: keep `daysUntil < 0` → error semantics. JSDoc on `classifyChipSeverity` now documents the boundary explicitly so future maintainers don't accidentally flip the comparison. [`kanbanThresholds.ts` `classifyChipSeverity`] (source: blind)
- [x] [Review][Decision] **No i18next plural-form variants (`_one`/`_other`) for `{{count}}` sub-line keys** — Resolved: added `_one` + `_other` variants for the 6 count-bearing keys in DE / IT / FR / ES (4 affected locales). Other locales keep the existing single-form keys (English needs no plural; Japanese has no plural distinction; Finnish/Dutch/Romansh/gsw-BE deferred to the wider native-speaker review pass). [`web-frontend/public/locales/{de,it,fr,es}/organizer.json`] (source: blind+edge)

#### Patches

- [x] [Review][Patch] **`now = nowProp ?? new Date()` per-render Date object → stale-closure in auto-clear effect** [`web-frontend/src/components/organizer/SpeakerStatus/SpeakerStatusLanes.tsx:309,372-388`] — `now` excluded from effect deps via `eslint-disable`; each render assigns a fresh `Date`; effect closure captures whichever `now` was assigned on the render that triggered it. At day-boundary refetches the effect can read against advancing `now`, causing non-deterministic auto-clear. Fix: derive a stable per-day key (e.g. `Math.floor(now.getTime() / MS_PER_DAY)`) and use it in deps, or compute `now` via `useMemo([renderTrigger])`. (source: blind+edge)
- [x] [Review][Patch] **`attentionSublines` reduction recomputed every render (no memoization) — ~800 classifier calls per render at scale** [`SpeakerStatusLanes.tsx:351-363`] — Author comment claims memoization is pointless because `now` always changes. Counter: `Math.floor(now.getTime() / MS_PER_DAY)` is stable within a day; memoizing on `[speakers, parsedEventDate, dayKey]` eliminates 99% of redundant classifier work. (source: blind+edge)
- [x] [Review][Patch] **Clickable sub-line buttons have no `aria-label` — screen readers read raw emoji + count, lose the toggle-filter semantic** [`SpeakerStatusLanes.tsx:687-711`] — Visible text starts with `⚠`/`⏰`/`📝`/`👀`/`🪑`; SR announces "warning sign 3 stale greater than 14 days, toggle button". Add `aria-label={t('organizer:speakerCard.lanes.<state>FilterAriaLabel', { count })}` with a descriptive translation. (source: blind+edge)
- [x] [Review][Patch] **READY sub-line `<Typography>` lacks `role="status"` / `aria-live` — gate-fired alert silent to SR** [`SpeakerStatusLanes.tsx:712-719`] — Static caption rendering "⚠ Slot capacity reached" with `color: warning.main`; sighted users see colour, SR users get nothing more than the text. Add `role="status"` (passive) or `role="alert"` (assertive) depending on urgency intent. (source: edge)
- [x] [Review][Patch] **No `aria-live` region for filter-toggle state change** [`SpeakerStatusLanes.tsx`] — `aria-pressed` flips on click but no announcement of "column filtered to N cards". Add a visually-hidden `aria-live="polite"` region updating with the filter result count. (source: edge)
- [x] [Review][Patch] **READY sub-line lacks `data-severity` attribute — asymmetric test contract vs clickable variants** [`SpeakerStatusLanes.tsx:712-719`] — All clickable sublines render `data-severity={chipSeverity}`; the static READY variant doesn't. Adding `data-severity="warning"` keeps the test contract uniform and unblocks deterministic Playwright assertions. (source: edge)
- [x] [Review][Patch] **Promote-to-READY E2E can fail unpredictably on slot-capacity gate** [`web-frontend/e2e/organizer/speaker-column-triage.spec.ts:391-405`] — Test seeds 3 speakers, transitions each via promote→INVITED→ACCEPTED. If event's default `maxSlots` is small, the second/third `promoteToReady` returns 409. `expect(resp.status()).toBeLessThan(300)` then throws. Fix: either inflate `maxSlots` for the seed event, or skip transitions once the gate fires. (source: blind)
- [x] [Review][Patch] **E2E `eventNumber` collision risk on parallel/back-to-back runs** [`speaker-column-triage.spec.ts:279`] — `String(900 + (Date.now() % 100))` yields 900-999 across a 100ms window; parallel workers collide. Use `Date.now()` directly or a UUID suffix. (source: blind)
- [x] [Review][Patch] **E2E uses `expect(resp.status()).toBeLessThan(300)` — accepts 1xx including 100/101** [`speaker-column-triage.spec.ts:337`] — Tighten to `expect(resp.status()).toBe(200)` or `>= 200 && < 300`. (source: blind)
- [x] [Review][Patch] **Inline arrow `onSublineClick={() => handleSublineClick(status)}` defeats memoization** [`SpeakerStatusLanes.tsx:1043,1055`] — Fresh function per lane per render. `useCallback`-wrap `handleSublineClick` and pass `status` via closure on a stable per-state handler map, or pass `status` as a data-attribute and read it inside a single handler. (source: blind)
- [x] [Review][Patch] **`data-severity = 'normal'` when timestamp is invalid silently looks like fresh data** [`SpeakerStatusLanes.tsx:1227-1235`] — Invalid timestamps should surface as `'unknown'` or omit the attribute. Treating bad data identically to fresh data hides backend bugs from QA. (source: blind)
- [x] [Review][Patch] **`now = new Date(NaN)` not guarded — silent "everything normal" lie** [`kanbanThresholds.ts:102`] — `classifyChipSeverity` guards `statusChangedAt` but not `now`. `diffDays(deadline, now)` with Invalid `now` returns `NaN`; all comparisons false → `'normal'`. Add `if (Number.isNaN(now.getTime())) return 'normal'` at function entry (or document the assumption). (source: edge)
- [x] [Review][Patch] **`parsedEventDate` from date-only ISO parses as UTC midnight → day-boundary off by hours in CET** [`SpeakerStatusLanes.tsx:311-315`] — `new Date('2026-05-17')` → `2026-05-17T00:00:00Z`. In CET/CEST, this is the day before locally. QUALITY_REVIEWED 14-day error boundary swings by ~2 hours. Anchor to local noon or use `parseISO` from `date-fns` for date-only strings. (source: blind+edge)
- [x] [Review][Patch] **QUALITY_REVIEWED stays grey while event detail is loading — flickers on resolve** [`SpeakerStatusLanes.tsx:309-315`] — `event?.date` undefined during query → `parsedEventDate = null` → QUALITY_REVIEWED chips/subline silently normal. Either pass loading-state through (skeleton chip) or accept the flicker; current behaviour is silent. (source: edge)
- [x] [Review][Patch] **`STATUS_LANES.reduce` accumulator cast lies about completeness — legacy union members fall through to `undefined`** [`SpeakerStatusLanes.tsx:351-363,303`] — `as Record<SpeakerWorkflowState, ...>` claims all 11 union members covered, but reduce only iterates the 8 ADR-009 states. `attentionFilter` typed as `SpeakerWorkflowState | null` permits legacy values. Narrow to `OutreachLane | PostAcceptanceLane` or use `Partial<Record<...>>`. (source: blind+edge)
- [x] [Review][Patch] **`countInvitedSplit` hardcodes `eventDate: null`, asymmetric signature vs `countAttentionCards`** [`kanbanThresholds.ts:2091-2113`] — Correct today (INVITED classification doesn't consult `eventDate`), but the asymmetry is a latent bug if future spec evolves INVITED classification to be event-date-aware. Either add `eventDate` to the signature or document why it's omitted in JSDoc. (source: blind+edge)
- [x] [Review][Patch] **AC7 grep invariant: `kanbanThresholds` not referenced in `SpeakerStatusLanes.test.tsx`** [`__tests__/SpeakerStatusLanes.test.tsx`] — Spec line 369 requires matches in all 4 expected files; the component test file imports nothing from `kanbanThresholds`. Add a token (import a constant or reference the module name in a comment) to satisfy the invariant. (source: auditor)
- [x] [Review][Patch] **`attentionMaxSeverity` exported but has zero direct tests** [`kanbanThresholds.ts:2132-2155`] — Public API surface unverified. Add `describe('attentionMaxSeverity')` block with cases: empty array → `'normal'`; mixed warning/error → `'error'`; all normal → `'normal'`. (source: blind)
- [x] [Review][Patch] **Mixed-slot QUALITY_REVIEWED test missing — predicate inversion would slip through** [`__tests__/kanbanThresholds.test.ts`] — Tests cover slot-assigned-true and slot-assigned-false in isolation; no test mixes both states against `countAttentionCards`/`makeAttentionPredicate`. Add a mixed-input case. (source: blind)
- [x] [Review][Patch] **Test #20 vacuously passes if all counts are zero** [`kanbanThresholds.test.ts:1790-1795`] — `expect(filtered.length).toBe(count)` passes when both sides are 0; predicate could be entirely broken. Add `expect(count).toBeGreaterThan(0)` for at least one state to guarantee non-trivial coverage. (source: blind)
- [x] [Review][Patch] **Boundary tests probe exact thresholds but not adjacent values (e.g. 29/59, not just 30/60)** [`kanbanThresholds.test.ts`] — `>=` boundary asymmetry untested; a flip to `>` would only break the exact-threshold cases (which are spec-ambiguous). Add adjacent-value tests (`29` → normal, `59` → warning) for at least IDENTIFIED, CONTACTED, READY. (source: blind)
- [x] [Review][Patch] **gsw-BE typo `Slot-Kapazitäät erreichi` (double ä)** [`web-frontend/public/locales/gsw-BE/organizer.json` `readySlotCapacitySubline`] — Likely machine-translation artefact; line 560 of the same file uses `Kapazität` (single ä). Fix to `Slot-Kapazität erreicht` (Bernese German). (source: blind)

#### Deferred (pre-existing or out-of-scope)

- [x] [Review][Defer] **`getStatusChangedAt` falls back to `updatedAt` for CONTACTED/READY/IDENTIFIED/QUALITY_REVIEWED — note edits reset the time-in-state clock** [`kanbanThresholds.ts:79-93`] — deferred, explicitly accepted as Resolved Q#2 from 11.D.2; backend `status_changed_at` column is the proper fix, out of scope here.
- [x] [Review][Defer] **`slotCapacity.reached` derived from in-memory speakers — breaks if pagination is added** [`SpeakerStatusLanes.tsx:333-345`] — deferred, already commented in code; switch to server-side count endpoint if pagination ever lands.
- [x] [Review][Defer] **INVITED with `responseDeadline === undefined` silently classifies as normal — no signal that the workflow has stalled** [`kanbanThresholds.ts:111-113`] — deferred; surfacing missing-deadline is its own UX concern, distinct from this story's threshold rules.
- [x] [Review][Defer] **INVITED `joiner` i18n key has no defensive code fallback — missing key would render the literal i18next key string** [`SpeakerStatusLanes.tsx:201`] — deferred; structural punctuation arguably should live in code, but the design pushes it to locales and all 10 currently carry it.
- [x] [Review][Defer] **`MS_PER_DAY = 86_400_000` constant ignores DST/leap-second days — `diffDays` off by one on transition days** [`kanbanThresholds.ts:51`] — deferred; rare boundary, `date-fns` `differenceInCalendarDays` is the proper fix when next touched.
- [x] [Review][Defer] **INVITED 1-hour boundary at deadline moment — `responseDeadline` as date-only ISO parses as UTC midnight** [`kanbanThresholds.ts:111-115`] — deferred; same root cause as the parsedEventDate TZ patch; tackle together with the date-only parsing rework.
- [x] [Review][Defer] **`Math.floor` asymmetry in `diffDays(later, earlier)` vs `-diffDays(earlier, later)` at sub-day deltas** [`kanbanThresholds.ts:51`] — deferred; semantically debatable, document in JSDoc when the date-only parsing rework happens.
- [x] [Review][Defer] **gsw-BE plural grammar — "1 überfellig" vs "2 überfelligi" not honoured** [`gsw-BE/organizer.json`] — deferred; same family as the plural-form decision above (machine-translated baselines flagged for native review anyway).
