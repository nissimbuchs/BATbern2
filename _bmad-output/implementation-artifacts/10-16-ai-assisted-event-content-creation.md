...
4. **AC4 — Abstract analysis**: `POST /api/v1/speakers/{speakerId}/ai/analyze-abstract` returns `{ noPromotionScore: 1-10, noPromotionFeedback: string, lessonsLearnedScore: 1-10, lessonsLearnedFeedback: string, wordCount: integer, shortenedAbstract: string|null }` from GPT-4o. Scores two BATbern-specific criteria: (1) absence of product promotion, (2) presence of real-world lessons learned. Provides optional shortened German abstract (≤150 words) when original exceeds 160 words.
...
10. **AC10 — Caching**: Results cached 1 hour by input hash via Caffeine (prevents duplicate API calls for identical inputs; max 500 entries). OpenAI-side rate limits apply for burst protection.
...
- [x] **T14 — Integrate abstract analysis into `QualityReviewDrawer.tsx`** (speaker abstract analysis)
  - [x] T14.1 — **Design pivot (Session 2)**: `AbstractAnalysisDrawer.tsx` was created then deleted. Analysis UI was merged into the existing `QualityReviewDrawer.tsx` (the natural home — organizer already reviews speaker content there). No separate component needed.
  - [x] T14.2 — `QualityReviewDrawer.tsx` now contains (gated on `aiContentEnabled` from `useFeatureFlags`):
    - "Abstract analysieren" / "Neu analysieren" button (hidden when AI disabled)
    - `noPromotionScore` and `lessonsLearnedScore` chips (color-coded: ≤4 red, ≤7 orange, ≥8 green)
    - Per-score German feedback text
    - Accordion: shortened abstract (when >160 words) with "Copy" button
  - [x] T14.3 — `useAiAnalyzeAbstract(speaker.id)` mutation wired into existing drawer state
...
- [x] **T16 — Abstract analysis accessible via `QualityReviewDrawer` (reached from `EventSpeakersTab`)** (AC: #4, #7)
  - [x] T16.1 — **Design pivot**: Abstract analysis button lives in `QualityReviewDrawer.tsx`, not `EventSpeakersTab.tsx` or `SpeakerOutreachDetailsDrawer.tsx`. Organizer opens QualityReviewDrawer from EventSpeakersTab (existing flow) → sees analyze button there.
  - [x] T16.2 — `useFeatureFlags` imported into `QualityReviewDrawer`; `aiContentEnabled` gates the button
  - [x] T16.3 — Button shown only when `abstract` is non-empty AND `aiContentEnabled` is true
...
- **T14 (design pivot)**: Initially created `AbstractAnalysisDrawer.tsx`; then deleted and merged analysis UI into existing `QualityReviewDrawer.tsx` — the natural home for abstract analysis (organizer already reviews content there). Abstract analyzer redesigned to score two BATbern-specific criteria: `noPromotionScore` (1-10) + `lessonsLearnedScore` (1-10), each with German feedback; optional `shortenedAbstract` (≤150 words) when original >160 words. Response schema differs from AC4 original spec — see updated AC4.
- **T15**: `EventOverviewTab.tsx` — added `useFeatureFlags` + `aiDrawerOpen` state; `AutoAwesome` button after description (gated on `aiContentEnabled && event.topicCode`); `AiAssistDrawer` mounted at bottom; `onDescriptionGenerated` → close drawer + navigate to edit
- **T16 (design pivot)**: `QualityReviewDrawer.tsx` — added `useFeatureFlags` + `useAiAnalyzeAbstract`; analyze button gated on `aiContentEnabled`; score chips + feedback + shortened abstract accordion. `EventSpeakersTab.tsx` and `SpeakerOutreachDetailsDrawer.tsx` not modified (no AI changes needed — QualityReviewDrawer already accessible via existing flow).
- Added `POST /events/{eventCode}/ai/theme-image/apply` endpoint: persists AI-generated `imageUrl` on event entity with Caffeine cache eviction; "Use this image" flow fixed. Also added `seed` query param to `/theme-image` to bust Caffeine cache on regenerate.
- **T17**: `aiAssist.*` keys added to de (German) + en (English); 8 other locales get German placeholders; all 10 locales valid JSON
- **T18**: type-check ✅; lint ✅; frontend tests ✅; Checkstyle ✅; backend ✅ (1464 tests, 1 pre-existing flaky performance test unrelated to 10.16)

**Session 3 (2026-03-03) — Code review fixes:**
- **H1**: Added `useFeatureFlags` to `QualityReviewDrawer.tsx`; gated analyze button on `aiContentEnabled` — AC1 now fully satisfied for all three AI UI entry points
- **H3**: Added `POST /events/{eventCode}/ai/theme-image/apply` to `docs/api/events-api.openapi.yml` + `ApplyThemeImageRequest` schema — ADR-006 compliance restored
- **L2**: Added two integration tests for `/apply` endpoint (404 + 401)
- **M1/M2**: Replaced stale i18n keys (`qualityScore`, `suggestion`, `keyThemes`, `improvedVersion`, `copyImproved`) with actual keys used by `QualityReviewDrawer` (`reanalyze`, `noPromotion`, `lessonsLearned`, `shortenedAbstract`, `words`, `copyShortened`) across all 10 locales
- **M3**: Added `eventCode` param to `BatbernAiService.generateEventDescription` + `generateThemeImage`; `ai_generation_log.event_code` now populated for description and image generation
- **AC4/AC10**: Updated story text to match actual implementation
...
- `web-frontend/src/components/organizer/SpeakerStatus/QualityReviewDrawer.tsx` (abstract analysis section: useFeatureFlags + useAiAnalyzeAbstract + score chips + shortened abstract accordion — **design pivot from T14/T16**: analysis merged here instead of separate AbstractAnalysisDrawer)
