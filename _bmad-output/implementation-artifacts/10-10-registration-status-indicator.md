...
- [ ] **T1 — OpenAPI spec first (ADR-006)** (AC: #1)
  - [ ] T1.1 — Add `GET /api/v1/events/{eventCode}/my-registration` to `docs/api/events.openapi.yml`
  - [ ] T1.2 — Define new schema `MyRegistrationResponse` with fields: `registrationCode: string`, `eventCode: string`, `status: string (enum: REGISTERED, CONFIRMED, WAITLIST, CANCELLED)`, `registrationDate: string (date-time)`
  - [ ] T1.3 — Specify security: bearer JWT required; 401 when missing; 404 when no registration found
  - [ ] T1.4 — Regenerate backend DTOs: `./gradlew :services:event-management-service:openApiGenerateEvents`
  - [ ] T1.5 — Regenerate frontend types: `cd web-frontend && npm run generate:api-types:events`

- [ ] **T2 — Repository query** (AC: #1)
  - [ ] T2.1 — Add `findByEventCodeAndAttendeeUsername` to `RegistrationRepository.java`:
    ```java
    ...
    ```
  - [ ] T2.2 — Add index hint comment: existing `idx_registrations_event_id` + `idx_registrations_attendee_username` cover this query

- [ ] **T3 — Service method** (AC: #1)
  - [ ] T3.1 — Add `getMyRegistration(String eventCode, String authenticatedUsername)` to `RegistrationService.java`
  - [ ] T3.2 — Returns `Optional<MyRegistrationResponse>` (generated DTO): builds from `Registration` entity, uppercases `status`, formats `registrationDate` as ISO-8601
  - [ ] T3.3 — Log: `DEBUG "Getting registration for event: {} and user: {}"`

- [ ] **T4 — Controller endpoint** (AC: #1)
  - [ ] T4.1 — Add `getMyRegistration` method to `EventController.java` (or extract to new `RegistrationController.java` if it exceeds 2500 lines — check current length first)
  - [ ] T4.2 — `@GetMapping("/{eventCode}/my-registration")`
  - [ ] T4.3 — Extract authenticated username from `SecurityContextHolder.getContext().getAuthentication().getName()`
  - [ ] T4.4 — Return `200` with `MyRegistrationResponse` when found; return `404` with standard `ErrorResponse` when not found
  - [ ] T4.5 — No `@PreAuthorize` needed — SecurityConfig already requires authentication for `GET /api/v1/events/**` non-public endpoints; confirm and add public path exception if needed

- [ ] **T4.6 — Allow re-registration for CANCELLED users** (AC: #6)
  - [ ] T4.6.1 — In `RegistrationService.createRegistration()` (line ~101), update the existing-registration check to distinguish `cancelled` from `confirmed`:
    ```java
    ...
    ```
  - [ ] T4.6.2 — Add integration test to `RegistrationStatusIntegrationTest` (T5): `POST /registrations` for a user whose existing registration is `CANCELLED` → `201` Created (new registration created successfully)
  - [ ] T4.6.3 — Add Checkstyle-compliant Javadoc to the updated block explaining the re-registration logic

- [ ] **T5 — Integration tests (TDD — write FIRST)** (AC: #9)
  - [ ] T5.1 — Create `RegistrationStatusIntegrationTest.java` extending `AbstractIntegrationTest` (PostgreSQL via Testcontainers)
  - [ ] T5.2 — Test: authenticated user with `CONFIRMED` registration → `200` + correct status
  - [ ] T5.3 — Test: authenticated user with `REGISTERED` registration → `200` + correct status
  - [ ] T5.4 — Test: authenticated user with `WAITLIST` registration → `200` + correct status
  - [ ] T5.5 — Test: authenticated user with `CANCELLED` registration → `200` + correct status
  - [ ] T5.6 — Test: authenticated user with no registration for event → `404`
  - [ ] T5.7 — Test: unauthenticated request → `401`
  - [ ] T5.8 — Run tests: `./gradlew :services:event-management-service:test --tests RegistrationStatusIntegrationTest 2>&1 | tee /tmp/test-10-10-backend.log`

### Frontend

- [ ] **T6 — `useMyRegistration` hook** (AC: #1, #7, #8)
  - [ ] T6.1 — Create `web-frontend/src/hooks/useMyRegistration.ts`
  - [ ] T6.2 — Signature: `useMyRegistration(eventCode: string | undefined)`
  - [ ] T6.3 — Uses `useAuth()` (existing hook); if `!isAuthenticated` OR `!eventCode` → return `{ data: undefined, isLoading: false }` immediately — NO API call
  - [ ] T6.4 — Uses `useQuery` from `@tanstack/react-query` with key `['my-registration', eventCode]` and `staleTime: 5 * 60 * 1000`
  - [ ] T6.5 — On `404` response: treat as "not registered" (return `{ data: null, isLoading: false }`) — do NOT throw error
  - [ ] T6.6 — Returns `{ data: MyRegistrationResponse | null | undefined, isLoading: boolean }`
  - [ ] T6.7 — Write `useMyRegistration.test.ts`: mock API, test authenticated flow (registered/not-registered/confirmed), test unauthenticated (no call made)
  - [ ] T6.8 — Wire cache invalidation in `RegistrationWizard.tsx` (AC: #7)
...
- [ ] **T7 — `registrationService` extension** (AC: #1)
  - [ ] T7.1 — Add `getMyRegistration(eventCode: string): Promise<MyRegistrationResponse>` to `web-frontend/src/services/registrationService.ts` (create file if it doesn't exist, following pattern of other service files)
  - [ ] T7.2 — Calls `GET /api/v1/events/{eventCode}/my-registration`; throws on non-404 errors; returns `null` on `404`

- [ ] **T8 — `RegistrationStatusBanner` component** (AC: #2, #3, #4)
  - [ ] T8.1 — Create `web-frontend/src/components/public/RegistrationStatusBanner.tsx`
  - [ ] T8.2 — Props: `status: 'REGISTERED' | 'CONFIRMED' | 'WAITLIST' | 'CANCELLED' | null | undefined`, `eventCode: string`, `isLoading: boolean`
  - [ ] T8.3 — If `isLoading === true`: render a skeleton `div` with same height as the banner (MUI `Skeleton` component, variant="rectangular", height="56px")
  - [ ] T8.4 — If `status === null || status === undefined`: render nothing (return `null`)
  - [ ] T8.5 — Otherwise render MUI `Alert` component:
  ...
  - [ ] T8.6 — Banner includes action link: for `CANCELLED` → i18n `registrationStatusBanner.registerAgain` → links to `/register/{eventCode}`; for all others → i18n `registrationStatusBanner.manageLink` → links to `/register/{eventCode}`
  - [ ] T8.7 — Add `data-testid="registration-status-banner"` to the Alert wrapper
  - [ ] T8.8 — Write `RegistrationStatusBanner.test.tsx`: test skeleton shown during loading, null returned when no status, each status variant renders correct severity and link

- [ ] **T9 — Integrate banner into `HomePage.tsx`** (AC: #2, #4)
  - [ ] T9.1 — Import and call `useMyRegistration(currentEvent?.eventCode)` where `currentEvent` is the existing "current event" loaded for the hero section
  - [ ] T9.2 — Place `<RegistrationStatusBanner>` immediately below the hero section (before the countdown timer if present, otherwise after the register button CTA area)
  - [ ] T9.3 — Only show banner when `currentEvent.status` is one of `AGENDA_PUBLISHED`, `AGENDA_FINALIZED`, `EVENT_LIVE` (do not show for past/archived events on homepage)

- [ ] **T10 — Integrate status chip into `EventCard.tsx`** (AC: #5)
  - [ ] T10.1 — Add optional prop `myRegistrationStatus?: string` to `EventCardProps` interface
  - [ ] T10.2 — When `myRegistrationStatus` is provided, render a `Badge` chip in the top-right of the card header (inside the `flex items-start justify-between` div, third flex child)
  - [ ] T10.3 — Chip colour mapping (use Tailwind classes):
  ...
  - [ ] T10.4 — Add `data-testid="event-card-status-chip"` to the chip element
  - [ ] T10.5 — Wire `myRegistrationStatus` prop in parent components — **strategy: N+1 per-event calls, parallelised with `Promise.all`, bounded to past 12 months** (BATbern runs ~6-10 events/year, so N ≤ 12 calls):
...
- [ ] **T11 — Registration Wizard guard** (AC: #6)
  - [ ] T11.1 — Find the event registration wizard component (search for component that handles `POST /api/v1/events/{eventCode}/registrations` on submit — likely in `web-frontend/src/components/public/` or `web-frontend/src/pages/public/`)
  - [ ] T11.2 — On wizard mount, call `useMyRegistration(eventCode)` (only when `isAuthenticated`)
  - [ ] T11.3 — If status is `REGISTERED`, `CONFIRMED`, or `WAITLIST`: show `RegistrationStatusGuard` inline component instead of the wizard form — displays current status + "You are already registered" + "Go back" button
  - [ ] T11.4 — If status is `CANCELLED`: show guard with "Register again" button that proceeds to step 1. The backend supports this: T4.6 updates `RegistrationService.createRegistration()` to delete the cancelled record and create a new one (do T4.6 before T11)
  - [ ] T11.5 — If status is `null` (not registered) or user not authenticated: show wizard normally (existing behaviour)

- [ ] **T12 — i18n keys** (AC: #10)
  - [ ] T12.1 — Add to `public/locales/en/registration.json`:
    ```json
    ...
    ```
  - [ ] T12.2 — Add `eventCard.statusChip.*` keys to `public/locales/en/events.json`:
    ```json
    "statusChip": {
      "confirmed": "Confirmed",
    ...
    ```
  - [ ] T12.3 — Add German translations for all keys to `public/locales/de/registration.json` and `public/locales/de/events.json`
  - [ ] T12.4 — Add `[MISSING]` prefix placeholder translations to all 8 other locale files (fr, it, rm, es, fi, nl, ja, gsw-BE)

- [ ] **T13 — Full test run** (AC: #9, #10)
  - [ ] T13.1 — `cd web-frontend && npm run test -- --run 2>&1 | tee /tmp/test-10-10-frontend.log && grep -c "pass\|fail" /tmp/test-10-10-frontend.log`
  - [ ] T13.2 — `npm run type-check 2>&1 | tee /tmp/typecheck-10-10.log`
  - [ ] T13.3 — `npm run lint 2>&1 | tee /tmp/lint-10-10.log`
  - [ ] T13.4 — Manual E2E (Playwright): navigate to homepage as authenticated confirmed user → green banner visible; navigate as unauthenticated → no banner
