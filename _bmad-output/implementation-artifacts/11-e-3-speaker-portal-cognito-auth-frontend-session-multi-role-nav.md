...
_Adversarial code review 2026-05-18 (bmad-code-review, Claude Opus 4.7 1M). Three parallel reviewers: Blind Hunter (adversarial), Edge Case Hunter (branch/boundary walker), Acceptance Auditor (AC-by-AC). Findings consolidated, deduped, classified._

**Auditor verdict roll-up: PASS 6 (AC1/AC2/AC3/AC5/AC8/AC9) | PARTIAL 5 (AC4/AC6/AC7/AC10/AC13) | FAIL 1 (AC12) | UNTESTED 1 (AC11, PR author owed).**

#### Decision-needed

- [x] [Review][Decision] **D1 — `ProfileUpdatePage` is broken: backend `/speaker-portal/.../profile/*` endpoints were deleted in Story 11.C.1, frontend still calls them, email service still builds magic-link-shaped `profileUrl`.** Frontend `speakerPortalService.ts:320-368` calls `GET /events/{eventCode}/profile`, `POST .../profile/photo/presigned-url`, `POST .../profile/photo/confirm` — none exist on backend (verified by `grep` over all `SpeakerPortal*Controller.java`; the `SpeakerPortalProfileController.java` + `SpeakerProfileService.java` + `/speaker-portal/profile*` Bruno tests were deleted by commit `e2b776e9` Story 11.C.1). `ProfileUpdatePage.tsx:70,78` calls `getProfile`/`updateProfile`; `App.tsx` still mounts `/speaker-portal/profile/:eventCode`; `SpeakerResponseService.java:228` builds relative `profileUrl = "/speaker-portal/profile/" + eventCode` (host-stripped — broken in emails); `SpeakerAcceptanceEmailService.java:135` still builds `baseUrl + "/speaker-portal/profile?token=" + viewToken` (legacy magic-link format → 404 on click). **Cannot ship as-is** — first speaker who clicks "Update Profile" in the acceptance email 404s. Decision: (a) re-add backend profile endpoints + fix email URL format, OR (b) delete `ProfileUpdatePage` + remove `/speaker-portal/profile` route + drop `profileUrl` from `SpeakerResponseResult` + remove the profile-link blocks from `speaker-acceptance-{de,en}.html` (profile is now consolidated into content-submit per Story 11.C.1's design).

- [x] [Review][Decision] **D2 — `SpeakerPortalAuthorizationService.resolveSpeakerPool` does not filter by event status; past-event content writes and accept/decline are reachable.** `SpeakerPortalAuthorizationService.java:34-47` returns the pool row for any event where the username matches, regardless of `Event.workflowState ∈ {EVENT_COMPLETED, EVENT_CANCELLED}`. A speaker with a bookmark to `/speaker-portal/respond/BATbern42` (long-past) can ACCEPT, content-submit, or upload materials. `SpeakerResponseService.checkAlreadyResponded` blocks ACCEPTED/DECLINED but not other terminal event states. Decision: (a) reject when event state is past/cancelled with a 409 or 403 + clear message, (b) keep as-is (workflow-state writer guards it elsewhere — verify), (c) defer to follow-up story.

- [x] [Review][Decision] **D3 — `SpeakerRoute` allows `['organizer', 'speaker']` but backend `@PreAuthorize("hasRole('SPEAKER')")` returns 403 for organizer-only users → mounts page then errors.** `web-frontend/src/components/auth/ProtectedRoute/ProtectedRoute.tsx:95-96` admits ORGANIZER through the frontend gate; backend strict-rejects. New `SpeakerPortalAuthIntegrationTest` explicitly asserts 403 for organizer-only tokens — backend contract is now SPEAKER-only. `ProtectedRoute.test.tsx:185` asserts the old permissive behaviour and is now misaligned. Decision: (a) tighten `SpeakerRoute` to `['speaker']` only and update the test, (b) keep the dual-role allowance and accept the 403 + add a friendlier error state, (c) keep but render a "this is a speaker-only page" message instead of mounting the data fetchers.

- [x] [Review][Decision] **D4 — `NavigationMenu` duplicates shared nav items (e.g., "Public Site") in every role section for multi-role users.** `NavigationMenu.tsx` + `NavigationMenu.test.tsx:222-238` — the test was renamed from `should_deduplicatePublicSite` to `should_showSharedItemsInEverySection_when_multipleRolesHaveSameItem`, deliberately locking in the duplication. A dual-role organizer+partner sees "Public Site" twice in the menu. The cherry-pick justification is "informative — tells the user the item belongs to both roles' nav." Concern: WCAG 2.4.4 (Link Purpose) — duplicate `aria-label`s with identical accessible names. Decision: (a) keep duplication, prefix `aria-label` per section (e.g., "Public Site (Organizer)"); (b) restore dedup for items that don't carry role-specific routes; (c) keep as-shipped per cherry-pick intent.

- [x] [Review][Decision] **D5 — AC12 (OpenAPI spec + frontend type regen) is explicitly deferred per Completion Note #35 — apply now or accept deferral?** `docs/api/speakers-api.openapi.yml` lines 557-621 still describe `/speaker-portal/content/submit` with `security: []` and the magic-link `token` body field; `web-frontend/src/types/generated/speakers-api.types.ts:201` still types the old endpoint. The implementation diverges from the canonical contract — direct violation of ADR-006 contract-first. Decision: (a) apply spec edits + regen types in this PR (substantial: ~10 endpoints across 5 DTOs, plus frontend type commit), (b) accept the dev's deferral and track as follow-up in `deferred-work.md`, (c) split into a sibling commit on the same branch before merge.

#### Patch (P0/P1 — should fix in this story)

- [x] [Review][Patch] **P1 — Username precheck inconsistent: only `submitContent` validates `speaker.username != null/blank`; the other four content+materials endpoints proceed** [`SpeakerPortalContentController.java:117-127`]. Lift the precheck into `SpeakerPortalAuthorizationService.resolveSpeakerPool` so every caller is gated once.
- [x] [Review][Patch] **P2 — `SpeakerPortalMaterialsService.confirmUpload` falls back to `speaker.getSpeakerName()` (display name) when `username` is null, writing display names into `uploadedBy`** [`SpeakerPortalMaterialsService.java:193`]. Mirror submit's fail-fast — drop the fallback and throw `ValidationException`.
- [x] [Review][Patch] **P3 — `SpeakerPortalTokenController` carries `@PreAuthorize("hasRole('SPEAKER')")` on a deprecated controller that takes a magic-link token in the request body — nonsensical combo.** Authenticated SPEAKERs would hit dead code; unauthenticated callers hit 401. Remove `@RequestMapping`/`@PostMapping` from the methods (controller class stays for Phase F) OR return `410 Gone` unconditionally [`SpeakerPortalTokenController.java`].
- [x] [Review][Patch] **P4 — `Authentication.getName() == null` causes `SecurityPrincipal` record constructor to throw `IllegalArgumentException`, which is unhandled → 500 instead of 401** [`SecurityPrincipal.java:62`; `GlobalExceptionHandler.java`]. Add `@ExceptionHandler(IllegalArgumentException.class)` for security-context-construction errors, or catch + remap inside `fromAuthentication`.
- [x] [Review][Patch] **P5 — Cherry-pick test files missing: `web-frontend/src/contexts/AuthContext.test.tsx` (+186 LOC) and `web-frontend/src/config/__tests__/navigationConfig.test.ts` (+94 LOC) are part of source SHAs `73d94688` + `396a9045` but not in this diff (verified by `ls`).** Re-apply from source commits.
- [x] [Review][Patch] **P6 — Dead-branch TENTATIVE residue: `InvitationResponsePage.tsx:323-324` compares `invitation.previousResponse === 'TENTATIVE'` after `SpeakerResponseType` was narrowed per Resolved Q#6.** Remove the dead branch + the `speakerPortal.invitationResponse.tentative` translation key sweep.
- [x] [Review][Patch] **P7 — `InvitationResponsePage` enters infinite loading state when `eventCode` not in `dashboard.upcomingEvents`** [`InvitationResponsePage.tsx:78-124`]. No branch handles `dashboard && !invitation` (also: page should search `pastEvents` and show "event is past" message, not silent spinner). Add explicit error state.
- [x] [Review][Patch] **P8 — `SpeakerPortalAuthIntegrationTest` accepts both 401 AND 403 for unauthenticated calls** [`SpeakerPortalAuthIntegrationTest.java:2196-2204,2210-2216`; also Bruno `32-dashboard-401-no-auth.bru`]. Pin to 401 (the actual Spring Security chain behaviour). Loose matrix masks future regressions.
- [x] [Review][Patch] **P9 — `SpeakerResponseService.processAcceptResponse` logs warn and continues when `speaker.username` is null instead of throwing/back-filling** [`SpeakerResponseService.java:1902-1908`]. Either back-fill from `actor.username()` (controlled write) or refuse the transition.
- [x] [Review][Patch] **P10 — `SecurityPrincipal.fromAuthentication` strips `ROLE_` prefix but Javadoc claims helper does it — prod path (no prefix) and test path (`WithMockUser` adds prefix) diverge silently** [`SecurityPrincipal.java`]. Pick one: always strip + update doc, or never strip and centralise in the resolver. Drop the silent divergence.
- [x] [Review][Patch] **P11 — Audit-column username can drift: `actor.username()` (Cognito `preferred_username`) ≠ `speaker.username()` (pool row, set at provisioning); `speaker_status_history.changed_by_username` records the Cognito spelling.** Use `speaker.getUsername()` (canonical, joins to `users.username`) for audit columns rather than `actor.username()` [`SpeakerResponseService.java`, `SpeakerWorkflowService.transition`].
- [x] [Review][Patch] **P12 — `gsw-BE` "Attendee" spelled inconsistently: `navigation.section.attendee = "Tailnähmer"` vs `role.attendee = "Teilnähmer"`** [`web-frontend/public/locales/gsw-BE/common.json`]. Standardise on the existing `Teilnähmer`.
- [x] [Review][Patch] **P13 — Stale class-level Javadoc on `SpeakerPortalContentController` references "AC1–AC4" per-method tags that were deleted from method docstrings in this diff.** Either restore per-method Javadoc or scrub the AC list from the class-level doc [`SpeakerPortalContentController.java:432-437`].
- [x] [Review][Patch] **P14 — `SpeakerDashboardPage.queryKey: ['speaker-dashboard']` lacks `user.username` — multi-tab logout-then-login as different speaker hits stale cache + PII leak** [`SpeakerDashboardPage.tsx:310`, also `InvitationResponsePage.tsx:72` shares the same key]. Include `user?.username` in the queryKey OR invalidate on auth-state change.
- [x] [Review][Patch] **P15 — Old email deep-links `/speaker-portal/respond?token=...&action=accept` no longer match the new route `/speaker-portal/respond/:eventCode` → 404.** Add a backward-compat redirect: `Route path="/speaker-portal/respond"` → redirect to `/speaker-portal/dashboard` (or to `/speaker-portal/respond/:eventCode` via URL inference if feasible) [`App.tsx`].

#### Patch (P2 — quality/test discipline)

- [x] [Review][Patch] **P16 — Bruno `33-respond-cognito-200.bru` accepts both 200 and 409, conflating "happy path" with "already responded" and failing for the wrong reason if seed breaks (403)** [`bruno-tests/speaker-portal-api/33-respond-cognito-200.bru:32-46`]. Split into two `.bru` files or add explicit pre-condition setup/teardown.
- [x] [Review][Patch] **P17 — Bruno `30-dashboard-200-speaker.bru` asserts `res.body.speakerName: isDefined` — empty string passes, but empty `speakerName` is exactly the bug from P23 (empty-pool fallback to username).** Tighten to `expect(body.speakerName).to.be.a('string').that.is.not.empty;`.
- [x] [Review][Patch] **P18 — `SpeakerPortalAuthIntegrationTest` 200-happy-path tests do not validate response body shape — only status code** [`SpeakerPortalAuthIntegrationTest.java`]. Add `jsonPath("$.upcomingEvents[0].eventCode").value(EVENT_CODE)` etc.
- [x] [Review][Patch] **P19 — `speaker-magic-login-404.spec.ts` runs only as authenticated speaker (gated by `SPEAKER_AUTH_TOKEN`); the realistic case (unauth visitor following an old email link) is never exercised** [`web-frontend/e2e/speaker/speaker-magic-login-404.spec.ts:11-21`]. Add an unauthenticated variant with `test.use({ storageState: { cookies: [], origins: [] } })`.
- [x] [Review][Patch] **P20 — `eventCode` `@PathVariable` is not length-bounded or pattern-validated** [`SpeakerPortalContentController.java:74,90,113,164,192`; same in dashboard/respond/materials]. A 5000-char path triggers a JOIN query with a giant string → potential DB error / 500 instead of clean 400. Add `@Pattern(regexp="^BATbern[0-9]+$")` or `@Size(max=12)`.
- [x] [Review][Patch] **P21 — `GlobalExceptionHandler.handleSpeakerPortalAccessDenied` returns 403 with no MDC correlation ID — support cannot tie the WARN log to the user-visible error** [`GlobalExceptionHandler.java:643-661`]. Generate correlation ID in the auth service and propagate via MDC.
- [x] [Review][Patch] **P22 — `submitContent` `ValidationException` log line uses `speaker.getId()` only — no `actor.username()` or `eventCode`, hard to correlate** [`SpeakerPortalContentController.java:611-620`]. Add both to the log line.
- [x] [Review][Patch] **P23 — `SpeakerDashboardService.getDashboard` returns `speakerName=username` when the pool is empty** [`SpeakerDashboardService.java:121-129`]. A speaker whose Cognito `preferred_username` is `alice.muller` sees "Welcome, alice.muller" instead of "Alice Müller". Look up display name from `UserApiClient` even on the empty-list path, or use a localised generic.

#### Defer

- [x] [Review][Defer] **DF1 — Five legacy speaker-portal `*IntegrationTest` classes marked `@Disabled`; net coverage delta vs new `SpeakerPortalAuthIntegrationTest` is debatable** — Phase F (Story 11.F.1) deletes the disabled files; new test focuses on auth matrix; behavioural-state coverage trade-off documented in Completion Note #13. Deferred — Phase F cleanup absorbs.
- [x] [Review][Defer] **DF2 — Four frontend Vitest suites + `speakerPortalService.test.ts` marked `describe.skip`** — same Phase F rationale; documented in Completion Notes #26-27. Deferred.
- [x] [Review][Defer] **DF3 — Three Playwright specs (`respond`, `content-submit`, `cross-portal-nav`) are `test.fixme` placeholders pending test-speaker seed (Story 11.E.2 staging dependency)** [`e2e/speaker/*.spec.ts`]. Explicitly tracked as a follow-up dev pass once the test-speaker user is provisioned in staging Cognito. Deferred.
- [x] [Review][Defer] **DF4 — Romansh / Spanish / Finnish / Dutch nav-section translations need native-speaker confirmation** (`Relatur`, `Ponente`, `Puhuja`, etc.) — non-blocking for ship, follow-up native review post-merge.
- [x] [Review][Defer] **DF5 — `docs/prd/epic-6-speaker-onboarding-plan.md` retains un-annotated current-behavior magic-link claims (7 lines)** — legacy doc, out-of-scope per Completion Note #36; add supersession banner in a follow-up sweep.
- [x] [Review][Defer] **DF6 — Belt-and-suspenders role check inside `SpeakerResponseService.processResponse` (independent of controller `@PreAuthorize`)** — controller layer covers; defensive duplication only needed if the service is reused outside speaker-portal context.
