...
| 2026-05-16 | `bmad-code-review` run by Nissim (3 layers — Blind Hunter, Edge Case Hunter, Acceptance Auditor). 2 decision-needed, 23 patch items, 5 deferred, 1 dismissed. See "Review Findings" below. |
| 2026-05-16 | Review patches applied: D1 (SPEAKER username throw), D2 (organizer resubmission test + doc), P0 (privilege escalation), 8 P1 (cache eviction, case-insensitive email, concurrent race, cross-event corruption, @NotBlank token, empty firstName/lastName), 14 P2 (validation, error-shape, AC compliance), 4 P3 (hygiene). Frontend types regenerated. 6 new Bruno contract tests authored. Full EMS suite: 1521/1521 PASSED. Full CUMS suite: 548/548 PASSED. Status → done. |

### Review Findings

_Code review by `bmad-code-review` on 2026-05-16. Layers: Blind Hunter (diff only), Edge Case Hunter (diff + project), Acceptance Auditor (diff + spec + ADR-009). Verdict: ⚠️ minor fixes — 1 P0 (privilege escalation) plus several P1 contract / idempotency gaps. 6/6 Resolved Decisions honored. 7 ACs met, 4 partial, 1 missing (AC10 Bruno tests, dev-acknowledged)._

**Decisions needed (resolved 2026-05-16 — converted to patches below):**

- [x] [Review][Decision→Patch] **SPEAKER principal username fallback** — Resolved: **throw `ValidationException` (fail closed)**. When `speaker.username` is null, the speaker-portal submission must reject with `"Speaker has no canonical username — please contact organizer to complete provisioning"`. The fallback to `speaker.getSpeakerName()` is removed. See P1 patch "[D1] SPEAKER principal username fallback — throw on missing username".
- [x] [Review][Decision→Patch] **Source-state precondition loosened on organizer endpoint** — Resolved: **keep loosened** (both endpoints accept `ACCEPTED || CONTENT_SUBMITTED`). Action: add an explicit organizer-resubmission integration test and update the AC4 doc + spec to document the loosened contract. See P2 patch "[D2] Organizer resubmission contract — add test + doc".

**Patches (unambiguous fixes — sorted by severity):**

_P0 — Privilege escalation_

- [x] [Review][Patch] **ORGANIZER can grant any role (incl. ORGANIZER) via `/users/provision`** — controller has `@PreAuthorize("hasAnyRole('ORGANIZER', 'ADMIN')")` but the service does NOT enforce a role-grant whitelist. An ORGANIZER caller can promote any user to ORGANIZER. Fix: enforce in `UserService.provisionUserWithRole` that for non-ADMIN principals, `request.role` must be in {SPEAKER} (or whatever Epic 11 needs); throw 403 otherwise. [UserController.java:676-689, UserService.java:794-840]

_P1 — Idempotency / data-correctness / contract / validation_

- [x] [Review][Patch] **`UserApiClientImpl.evictUserCache` self-invocation bypasses Spring's `@CacheEvict` proxy** — `provisionUserWithRole` calls `this.evictUserCache(username)`; AOP proxy is NOT in play → cache is never evicted despite the javadoc claim. Once 11.E.2 wires the caller, downstream `getUserByUsername` reads stale data. Fix: annotate `provisionUserWithRole` itself with `@CacheEvict(value="userApiCache", key="#result?.username", condition="#result != null")`, OR inject `CacheManager` and call `cacheManager.getCache("userApiCache").evict(username)` programmatically. [UserApiClientImpl.java:497, 609-614]
- [x] [Review][Patch] **`UserService.provisionUserWithRole` lookup not case-insensitive** — javadoc / comment claim "case-insensitive on lower(email) — same path as getOrCreateUser", but the code calls `userRepository.findByEmail(...)` which is a plain Spring Data derived query (case-sensitive). Idempotency claim is false: `Jane@x.com` then `jane@x.com` create two User rows. Fix: use `findByEmailIgnoreCase` (add to repository if missing) or normalize email to lowercase before lookup AND before persist. Add a regression test that toggles case. [UserService.java:806]
- [x] [Review][Patch] **Concurrent `provisionUserWithRole` race on the same email produces 500 instead of idempotent response** — check-then-act between `findByEmail` and `createNewUser`; two parallel CONTACTED → READY transitions both hit "empty" then one succeeds, the other gets `DataIntegrityViolationException` → no handler → 500. Fix: add a `try { createNewUser } catch (DataIntegrityViolationException) { /* re-fetch, treat as idempotent existing-user case */ }` around the create, OR add a `@Recover` annotation that re-fetches and returns. [UserService.java:806-840]
- [x] [Review][Patch] **Cross-event content submission corruption — organizer endpoint does not verify `speaker.eventId` matches `eventCode` path param** — service loads speaker by `speakerId`, then `getOrCreateSession` writes `event_id = speaker.getEventId()` and `event_code = <path>`; if a stale or mistyped URL pairs a speaker from BAT-002 with `eventCode = BAT-001`, the session row's `event_code` ↔ `event_id` are inconsistent and downstream `event_code`-keyed consumers misroute. Fix: in `ContentSubmissionService.submit`, resolve the Event by `eventCode` and assert `event.getId().equals(speaker.getEventId())`; reject 400 (or 404 if event not found) on mismatch. [ContentSubmissionService.java:292-294, 437-445]
- [x] [Review][Patch] **Magic-link `token` field has no `@NotBlank` on `ContentSubmitRequest`** — OpenAPI marks `token` required, but the Java record has no jakarta.validation constraints. Null/empty token reaches `MagicLinkService.validateToken(null)` and yields either NPE → 500 or misleading "Invalid token". Fix: add `@NotBlank String token` to the record component, or add an explicit null/blank guard in the controller. [ContentSubmitRequest.java:23-31, SpeakerPortalContentController.java:1615]
- [x] [Review][Patch] **`provisionUserWithRole` with empty firstName/lastName returns 500** — both fields are optional in the OpenAPI spec (only `email` + `role` required) but `createNewUser` → `slugService.generateUsername("","")` throws `IllegalArgumentException` → falls through to generic exception handler → 500. Fix: either (a) require firstName/lastName in the OpenAPI required list, (b) derive default firstName/lastName from email local-part when absent, or (c) catch the IllegalArgumentException and return 400 with a clear message. [UserService.java:823-829]

_P2 — Validation, error shape, spec/impl drift_

- [x] [Review][Patch] **`additionalProperties: false` silently bypassed on CUMS endpoints** — generated `ProvisionUserRequest`/`PatchUserProfileRequest` use `@JsonAnySetter` and accept unknown fields. Resolved Decision §3 + ADR-009 claim 400 on stale fields; this only holds for the manually-maintained EMS DTOs. Fix: customize the OpenAPI generator template to emit `@JsonIgnoreProperties(ignoreUnknown=false)` on `additionalProperties: false` schemas, OR add a controller-level filter, OR drop the spec claim. Add an integration test asserting 400 for stale fields against `/users/provision`. [build/generated-users/.../ProvisionUserRequest.java:194]
- [x] [Review][Patch] **`ProvisionUserRequest.username` accepted on API surface but silently ignored** — service never reads `request.getUsername()`; user is always created with `slugService.generateUsername(firstName,lastName)`. Fix: either honor the caller-supplied username (with validation) or remove `username` from the request schema until 11.E.2 needs it. [UserService.java:794-840]
- [x] [Review][Patch] **Speaker portal lacks Bean Validation on `bio` / `profilePictureUrl`** — organizer endpoint enforces `@Size(max=5000)`/`@Size(max=2048)` on `SubmitContentRequest`, but speaker portal record `ContentSubmitRequest` has no `@Size`. Long bio reaches CUMS which 400s, but EMS's `UserApiClient.patchUserProfile` re-throws as `UserServiceException` with no handler → 500 to the speaker. AC8 "identical downstream effects" violated. Fix: add `@Size` constraints to `ContentSubmitRequest` matching `SubmitContentRequest`, OR add a `@ExceptionHandler(UserServiceException.class)` that maps 400→400. [ContentSubmitRequest.java:23-31]
- [x] [Review][Patch] **PATCH `/users/{u}/profile` with `{"bio": ""}` silently clears bio** — `UserService.patchUserProfile` null-checks both fields but accepts empty strings, then writes `user.setBio("")`. Consolidated `ContentSubmissionService.maybePatchUserProfile` correctly skips blanks; direct PATCH does not. Fix: in `UserService.patchUserProfile`, treat blank strings as "no change" (skip the setter) OR throw 400 ValidationException on blank-but-non-null. [UserService.java:863-905]
- [x] [Review][Patch] **Test weakened from explicit 400 to `is4xxClientError()`** — `SpeakerStatusControllerIntegrationTest "should return error when speaker not in ACCEPTED state"` accepts any 4xx; the test's own comment admits "could be 400 or 422 depending on mapper". Clients need a deterministic code. Fix: pin the `IllegalStateException` → status mapping (recommendation: 422 Unprocessable Entity for state-machine violations, 400 for malformed input); restore `status().isUnprocessableEntity()` (or `.isBadRequest()`) in the test. [SpeakerStatusControllerIntegrationTest.java:~3046]
- [x] [Review][Patch] **`presentationUploadId` accepted on API surface but silently NOT linked** — `ContentSubmissionService.submit` only logs `"deferred to Story 11.D.4"`. Caller sees 201 with their deck reference but nothing is associated. AC4 step 8 spec said "wired through". Fix: either (a) reject the field with 400 ("not supported until 11.D.4") until linked, OR (b) remove `presentationUploadId` from both request schemas until 11.D.4 lands, OR (c) at minimum echo it back in the response with a warning. [ContentSubmissionService.java:337-340]
- [x] [Review][Patch] **[D1] SPEAKER principal username fallback — throw on missing username** — (resolved decision D1) In `SpeakerPortalContentController.submitContent`, replace the `speaker.getSpeakerName()` fallback with `throw new ValidationException("Speaker has no canonical username — please contact organizer to complete provisioning")`. Add integration test asserting 400 for the legacy `username == null` case. Updates AC6 dev note accordingly. [SpeakerPortalContentController.java:195-198]
- [x] [Review][Patch] **[D2] Organizer resubmission contract — add test + doc** — (resolved decision D2) The new shared `submit()` accepts `ACCEPTED || CONTENT_SUBMITTED` for both endpoints — keep as-is. Action: (a) add an integration test in `SpeakerStatusControllerIntegrationTest` proving organizer resubmission from CONTENT_SUBMITTED succeeds and writes a self-transition history row; (b) update AC4 in this spec + the OpenAPI description for `POST /events/{code}/speakers/{speakerId}/content` to document the loosened contract. [ContentSubmissionService.java:2244, SpeakerStatusControllerIntegrationTest.java, docs/api/speakers-api.openapi.yml]
- [x] [Review][Patch] **`EntityNotFoundException` returns 500 instead of 404** — `ContentSubmissionService.submit` throws `jakarta.persistence.EntityNotFoundException` when speaker not found; javadoc claims "404 if missing"; EMS `GlobalExceptionHandler` has no handler for `EntityNotFoundException` → 500. Fix: add `@ExceptionHandler(EntityNotFoundException.class)` returning 404, OR change the throw to `NotFoundException`/`SpeakerNotFoundException` which IS handled. [ContentSubmissionService.java:293]
- [x] [Review][Patch] **`ProvisionUserResponse.username` pattern in response schema rejects valid existing usernames** — response carries `pattern: '^[a-z]+\.[a-z]+(\.[0-9]+)?$'` which rejects digit-prefixed, hyphenated, or apostrophe names. If an existing user has a non-conforming legacy username, OpenAPI-validated clients reject the response → 500 at the caller. Fix: remove `pattern` from response schemas (responses describe what the server emits, not constraints); keep on requests if useful. [users-api.openapi.yml:547, 586]
- [x] [Review][Patch] **`getOrCreateSession` mutates `speaker.sessionId` without explicit save** — relies on JPA flush-on-commit. Works today because `submit()` is `@Transactional` and loads the speaker in the same context, but if anyone later extracts the helper (likely in 11.D.4), the mutation will silently drop. Fix: add explicit `speakerPoolRepository.save(speaker)` immediately after `setSessionId(...)`, or refactor the contract. [ContentSubmissionService.java:2381, 2431]
- [x] [Review][Patch] **Cross-service dual-write on profile patch — CUMS commits independently of EMS rollback** — `maybePatchUserProfile` runs inside `@Transactional` BEFORE the state transition; if `transition()` fails after CUMS has already committed the bio, EMS rolls back but the bio is patched. Javadoc claims atomic semantics — it isn't. Fix per Open Question §5: move the call to a `@TransactionalEventListener(AFTER_COMMIT)` (accept eventual consistency); OR honestly update the javadoc to flag the asymmetry. [ContentSubmissionService.java:2451-2475]

_P2 — Acceptance-Auditor-only_

- [x] [Review][Patch] **AC10 Bruno API contract tests skipped** — dev-acknowledged trade-off. Missing: `bruno-tests/users-api/provision-user.bru`, `patch-user-profile.bru`, plus stale-fields-rejection cases on both content endpoints. Fix: author the 5 `.bru` files and run `./scripts/ci/run-bruno-tests.sh` green. [bruno-tests/]
- [x] [Review][Patch] **Frontend types not regenerated (AC2 / AC11 explicit)** — `web-frontend/src/types/generated/users-api.types.ts` and `speakers-api.types.ts` are not updated despite the spec requiring it. Fix: `cd web-frontend && npm run generate:api-types` and commit. [web-frontend/src/types/generated/]
- [x] [Review][Patch] **Doc drift in `06a-workflow-state-machines.md:286`** — still references the non-existent `submitted_by_username` column (per Resolved Decision §2 / Task 9.3). Fix: one-line edit to remove the stale mention. [docs/architecture/06a-workflow-state-machines.md:286]
- [x] [Review][Patch] **Missing explicit AC9 item #6 equivalence test** — dev acknowledges; covered indirectly. Fix: add a test that submits the same payload via both endpoints and asserts `ContentSubmission`/`Session`/`SpeakerPool` rows are byte-equal modulo `changed_by_username`. [ContentSubmissionServiceIntegrationTest.java]
- [x] [Review][Patch] **Missing ATTENDEE → 403 test on `PATCH /users/{u}/profile`** — spec AC2 test list item (d) was not authored; only the controller-level `@PreAuthorize` enforces it. Fix: add a test case in `UserProvisioningAndPatchIntegrationTest`. [UserProvisioningAndPatchIntegrationTest.java]
- [x] [Review][Patch] **`sprint-status.yaml` scope-creep flip `epic-8: in-progress → done`** — unrelated to 11.C.2; minor. Fix: revert the line OR split into its own commit with justification. [_bmad-output/implementation-artifacts/sprint-status.yaml]

_P3 — Minor consistency / hygiene_

- [x] [Review][Patch] **`SubmitContentRequest.@Size(max=255)` on `presentationTitle` exceeds service constant `MAX_TITLE_LENGTH=200`** — Bean Validation passes 201-255 chars then service throws `IllegalArgumentException` → 400 with non-standard error shape. Fix: align `@Size(max=200)` with the service constant and the OpenAPI spec (which says `maxLength: 200`). [SubmitContentRequest.java:44]
- [x] [Review][Patch] **Empty patch `{}` 400 shape inconsistent with Bean Validation** — `UserService.patchUserProfile` throws `UserValidationException("At least one of 'bio' or 'profilePictureUrl' must be present")` from service layer; clients parsing Bean-Validation error envelopes see a different shape. Fix: add `@AssertTrue` on the DTO ("at least one field non-null") so the 400 comes from `MethodArgumentNotValidException` and shares the envelope shape. [UserService.java:864-869]
- [x] [Review][Patch] **No integration test for stale-fields rejection on speaker portal endpoint** — only the organizer endpoint has the `should_return400_when_legacyFieldsPresent` test; the speaker portal `ContentSubmitRequest` is untested for the same contract. Fix: add the equivalent test in `SpeakerPortalContentControllerIntegrationTest`. [SpeakerPortalContentControllerIntegrationTest.java]
- [x] [Review][Patch] **`RoleService.addRole` idempotency not asserted at DB-row level in test** — `should_beIdempotent_when_provisionCalledTwice` checks `Set<Role>` membership but not `role_assignments` row count. Fix: assert `roleAssignmentRepository.countByUsernameAndRole(...) == 1` (add the count method if missing). [UserProvisioningAndPatchIntegrationTest.java]

**Deferred (pre-existing or low-impact — logged in `deferred-work.md`):**

- [x] [Review][Defer] **`SpeakerWorkflowService.writeHistoryRow` writes `Instant.now()` — same-instant rows have identical `changed_at`** [SpeakerWorkflowService.java:380] — pre-existing pattern; PostgreSQL TIMESTAMP microsecond precision makes this rare in real workflows; addressable via tiebreak column if test flakiness emerges.
- [x] [Review][Defer] **SPEAKER self-patch fails 403 on case-mismatched username path** [UserController.java:723] — pre-existing convention; usernames are stored canonical-case at Cognito provisioning; addressable when Phase E lands.
- [x] [Review][Defer] **`magicLinkService.markTokenAsUsed` for SUBMIT tokens is functionally no-op** [SpeakerPortalContentController.java:216] — `MagicLinkService.validateToken` only checks `is_used` for RESPOND tokens; the new call is misleading but harmless. Will be removed in Phase F magic-link teardown.
- [x] [Review][Defer] **`RestTemplate.exchange` for provisionUserWithRole does not error on 200-with-null-body** [UserApiClientImpl.java:1378-1391] — paranoia; CUMS controller never returns null body and OpenAPI-validated client/server pair enforces non-null response. Address only if observed in production.
- [x] [Review][Defer] **Async `SpeakerContentSubmittedEvent` listener failure may drop organizer notification** [ContentSubmissionService.java:2348] — Blind Hunter speculation; out-of-scope for this story. Confirm retry/DLQ semantics in a separate observability story before MVP.

**Dismissed as noise:** 0 (all findings retained).
