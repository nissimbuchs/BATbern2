...
- **Exception: `role_assignments.role` is stored UPPER_CASE**, not lowercase. Its
  ...
  hand-written `INSERT … 'speaker'` is rejected. Verified 2026-09-09 by querying **production**
  (account 188701360969): `role_assignments.role` holds `ORGANIZER|SPEAKER|PARTNER|ATTENDEE`,
  while `events.workflow_state` (`archived`, `slot_assignment`) and `speaker_pool.status`
  (`contacted`, `quality_reviewed`) do follow the lowercase rule — `role` is the one that
  does not. Local dev matches production on all three.
