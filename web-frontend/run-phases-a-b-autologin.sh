#!/bin/bash
# Phase A + B test runner with autologin support
# Autologin handles authentication - no password needed!

set -e

echo "🎬 Phase A + B Test Runner (Autologin)"
echo "======================================="
echo ""
...
# Run Phase A + B tests
echo "🧪 Running Phase A + B tests (serial execution)..."
if [ "$UI_MODE" = true ]; then
...
# Note: Using -g to match both "Phase A" and "Phase B"
...
eval "$PLAYWRIGHT_CMD" 2>&1 | tee /tmp/phase-a-b-test-run.log
...
if [ $TEST_RESULT -eq 0 ]; then
  ...
  echo "❌ Phase A + B tests FAILED"
  echo ""
  echo "Full output saved to: /tmp/phase-a-b-test-run.log"
  echo ""
  echo "Check error screenshot:"
  echo "   ls -lrt test-results/**/test-failed-*.png | tail -1"
  echo ""
  echo "Quick error check:"
  echo "   grep -A 5 'Error:' /tmp/phase-a-b-test-run.log | tail -20"
fi
