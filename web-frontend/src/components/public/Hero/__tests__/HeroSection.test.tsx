...
vi.mock('@/components/public/Registration/RegistrationWizard', () => ({
  RegistrationWizard: ({ eventCode, inline, onCancel }: { eventCode: string; inline: boolean; onCancel?: () => void }) => (
    <div data-testid="registration-wizard">
      ...
      {onCancel && <button onClick={onCancel} data-testid="wizard-cancel">Cancel</button>}
    </div>
  ),
}));
...
describe('HeroSection Component', () => {
  ...
  describe('Inline Registration (Story 4.1.5)', () => {
    ...
    test('should_showOverlay_when_wizardExpanded', async () => {
      ...
      let overlay = container.querySelector('[class*="bg-zinc-950/70"]');
      ...
      await waitFor(() => {
        overlay = container.querySelector('[class*="bg-zinc-950/70"]');
    ...
    test('should_changeHeight_when_wizardExpanded', async () => {
      ...
      await waitFor(() => {
        expect(section).toHaveStyle({ minHeight: 'auto' });
      });
    });
  });
});
