#!/bin/bash
# Phase A + B + C test runner with autologin support
# Autologin handles authentication - no password needed!

set -e

echo "🎬 Phase A + B + C Test Runner (Autologin)"
echo "============================================"
echo ""
...
echo "🧪 Running Phase A + B + C tests (serial execution)..."
if [ "$UI_MODE" = true ]; then
...
# Note: Using -g to match "Phase A", "Phase B", "Phase B.5", and "Phase C"
...
eval "$PLAYWRIGHT_CMD" 2>&1 | tee /tmp/phase-a-b-c-test-run.log
...
if [ $TEST_RESULT -eq 0 ]; then
  ...
  echo "❌ Phase A + B + C tests FAILED"
  echo ""
  echo "Full output saved to: /tmp/phase-a-b-c-test-run.log"
  echo ""
  echo "Check error screenshot:"
  echo "   ls -lrt test-results/**/test-failed-*.png | tail -1"
  echo ""
  echo "Quick error check:"
  echo "   grep -A 5 'Error:' /tmp/phase-a-b-c-test-run.log | tail -20"
fi
