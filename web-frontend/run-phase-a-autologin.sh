#!/bin/bash
# Phase A test runner with autologin support
# Autologin handles authentication - no password needed!

set -e

echo "🎬 Phase A Test Runner (Autologin)"
echo "==================================="
echo ""
...
eval "$PLAYWRIGHT_CMD" 2>&1 | tee /tmp/phase-a-test-run2.log
...
if [ $TEST_RESULT -eq 0 ]; then
  ...
  echo ""
  echo "Full output saved to: /tmp/phase-a-test-run2.log"
  echo ""
  echo "Check error screenshot:"
  echo "   ls -lrt test-results/**/test-failed-*.png | tail -1"
  echo ""
  echo "Quick error check:"
  echo "   grep -A 5 'Error:' /tmp/phase-a-test-run2.log | tail -20"
fi
