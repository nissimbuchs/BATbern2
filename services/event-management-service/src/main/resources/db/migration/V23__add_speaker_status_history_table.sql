-- V23__add_speaker_status_history_table.sql
-- Story 5.4 & 5.5: Speaker Status Management with Quality Review
...
CREATE TABLE IF NOT EXISTS speaker_status_history (
    ...
    session_id UUID,  -- Story 5.5: NULL until speaker submits content (CONTENT_SUBMITTED status)
...
-- Make session_id nullable for existing tables (staging compatibility)
-- This handles cases where table was created with session_id NOT NULL in earlier deployments
ALTER TABLE speaker_status_history ALTER COLUMN session_id DROP NOT NULL;
...
COMMENT ON TABLE speaker_status_history IS 'Status transition history for speakers during event planning (Workflow Step 5) - Story 5.4 & 5.5';
...
COMMENT ON COLUMN speaker_status_history.session_id IS 'Session ID - NULL until speaker submits content (CONTENT_SUBMITTED status) - Story 5.5';
