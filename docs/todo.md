...
- [x] not all ids are translated in topics backlog
...
- [x] history diagram not shown but implemented
- [ ] TODO: story-1.16-workflow-visualisation wirframe was never implemented. shows up when you press on the progress bar
- [x] import topics does not work. mixes up cathegories from the json file (which are topics) with cathegories of topics in the api. thus it uses non existing cathegories in the api. Also, as all events are archived, creates illegal state transition. should be able to override if topics even on archived topics.
- [ ] errors from the statemachine dont use the shared kernel format
- [ ] event dashboard status filter has wrong stati. change year filter to be a textbox that allows any year
