...
> the heaviest) MERGED — squash `0c2f9789` (PR #701; its deploy-to-staging deploy/Bruno/Playwright
> `@smoke`/promote all ran green — the first run hit a transient `BATbern-staging-Frontend`
> CFN `UPDATE_IN_PROGRESS` collision with the prior deploy, fixed by a plain re-run once the stack
> settled). Green ×2 vs dev = 14/14, residue-free. Render `@gate` for all 9 tabs + 2 pages
