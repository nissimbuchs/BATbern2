...
> **Where we are (2026-05-27 ~10:00 CET):** **PRs 670, #671, #672 (PR 13) all MERGED** to `develop` (`#672` = `36b3e3dc`). 🔴 **HOTFIX IN FLIGHT (`fix/partner-meetings-bruno-no-real-invites`):** PR 13's `08-send-invite.bru` exercised the REAL `POST /send-invite`, which dispatched live calendar invitations to every partner contact via SES on the production account — and the follow-up DELETE sent a real cancellation. **This shipped in #672 and ran on staging — real partners were emailed.** The fix removes the send-invite test from the collection (create/get/list/update/delete remain; partner-meetings-api now 14 req / 22 tests, green locally); send-invite stays covered only at the mocked integration-test layer. **Binding rule added:** no Bruno/E2E test may trigger real outbound email/invites/notifications against staging (= production). Also: the manual run #2 (`build.yml --ref develop` workflow_dispatch) **failed at web-frontend** because API types weren't regenerated in that path — so green ×2 on staging is NOT yet confirmed via that run. **PR 14 (gate flip) committed locally on `test-enhancement/bruno-gate-flip` but HELD** until the hotfix lands + green ×2 is properly re-confirmed + the F1 drill is run. Original PR 13 summary follows:
>
