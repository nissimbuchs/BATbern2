...
> **Where we are (2026-05-27 ~17:00 CET):** **PRs 670, #671, #672, #673, #675, #676 all MERGED** to `develop`; **#677 OPEN** (CLAUDE.md "never modify an applied Flyway migration" rule). **The real "staging keeps breaking after Epic 11" is FIXED.** Removing the Bruno `continue-on-error` mask (PR #674) exposed that **EventManagement had been stuck on stale build 1142 since #669** — #669's bulk `cdn.staging.batbern.ch → cdn.batbern.ch` substitution accidentally edited the already-applied `V86`, so EMS failed Flyway validation on boot and the ECS circuit-breaker kept rolling it back. **#676 fixed it** (revert V86 byte-exact + new `V105` forward migration): **EMS deployed off 1142 → build 1159**, `/event-types` works, and **`event-types-api` is GREEN on staging (9/9, verified by running the collection from here)**. The never-firing **`staging-stable` promote ran** for the first time. (Earlier fixes in #675: gateway `/api/v1/event-types` route + `rollback-deployment.sh` PascalCase ECS service-name resolution.) **PR #674 (gate flip) REBASED onto current develop and re-attempting** — removes step-level `continue-on-error`; downstream jobs gate on `needs.bruno-tests.result`; `rollback-on-bruno-failure` gains `always()` + drops the `enable_bruno_rollback` gate. Its deploy now runs the **blocking** gate against a genuinely-green suite → should pass + merge. **NEXT:** confirm #674 green/merged, then the **F1 deliberate-fail drill** (real production rollback — low-traffic-window heads-up first; the rollback script is now functional per #675). Then the hardening plan is complete.
...
| 2 | `test-enhancement/bruno-events-api-split` (= PR 670, 10 commits) | Section C mechanical-only: file moves + cleanup hooks for 4 active collections + 2 stubs (event-types-api, event-full-workflow-api gets seeded with tests 12/13/16). Net-new tests deferred to PR 2a + PR 2b. **+ bundled apex-domain hotfix from #669 fallout** | ✅ merged | 2026-05-26 (#670, squash `57d80f14`) | events-api was 49/60 on staging pre-split — splitting is the unlock for PRs 6 (batched audit) + 11 + 12. **Bundled hotfix (claude-review on #669, unfixed at merge):** `RateLimitingFilter.java:155` + `TurnstileVerificationFilter.java:199` both ended up with `origin.equals("https://www.batbern.ch") \|\| origin.equals("https://www.batbern.ch")` (apex `https://batbern.ch` dropped from the OR). Fix: both filters now delegate via `corsHandler.isOriginAllowed(origin)`. Branch deleted post-merge (local + remote). |
| 2a | (merged in #670) | Net-new event-types-api tests: 9 Bruno tests (GET list + GET specific + GET invalid + idempotent PUT capture-then-replay + PUT validation 400 + POST/DELETE 404/405). **+ bundled backend hygiene fix**: `HttpRequestMethodNotSupportedException` handler in all 4 services' `GlobalExceptionHandler` (EMS, CUMS, PCS, SCS) — without it, POST/DELETE on a GET-only route returned 500 (same gotcha class as the `MethodArgumentNotValidException` rule in project-context.md). | ✅ merged | 2026-05-26 (#670, `57d80f14`) | 19/19 tests green ×2 locally. |
| 2b | (merged in #670) | Net-new event-full-workflow-api tests: 7-step state-machine walk CREATED → ARCHIVED via PUT `/events/{code}/workflow/transition` with `overrideValidation:true` + 2 invalid-transition rejection tests + 3 fixture events + 2 cleanup hooks. Registrations CRUD against `REGISTRATION_OPEN` deferred to potential PR 2c — not blocking. | ✅ merged | 2026-05-26 (#670, `57d80f14`) | 44/44 tests green ×2 locally. |
| 2c | (merged in #670) — DB cleanup + 4 local-dev test fixes | `scripts/db/bruno-dev-pre-cleanup-2026-05-26.sql` one-shot: deleted 88 user_profiles + 14 companies + 3 test events (cascading 14 sessions, 12 speaker_pool, 30 registrations, 18 session_content_history, 1 newsletter_send) + 1 partner. Preserved 11 real Bruno-named people + 2 real partner_meetings. Plus 4 test fixes: users-api/15 timestamp-collision subprefix, sessions-api/21 stale presentationTitle assertion (Story 11.E.8 V100 dropped the column), speaker-portal-api/03+04 magic-login deleted endpoints now accept 401-or-404. | ✅ merged | 2026-05-26 (#670, `57d80f14`) | Local Bruno: 11/13 collections green, 245/247 tests (99.2%). |
...
| 6 | `test-enhancement/bruno-audit-batched-closeout` (= PR #671, bundled with PR 11) | **Batched audit-pass closeout** — Section D steps 2 + 4 across tasks-api + events-crud-api + event-topics-api + sessions-api + event-full-workflow-api. tasks-api gets the canonical hook migration (00/00b/98/99); other four get DELETE-then-GET-404 verify stubs (29a, 67a, 11a, 97c). | 🔵 in review (PR #671 → develop) | — | **Local Bruno green ×2 on all 5 collections** (tasks-api 31/31, events-crud-api 30/30, event-topics-api 26/26, sessions-api 58/58, event-full-workflow-api 46/46; re-verified post-rebase). **Step 2 finding (verified during impl):** `CreateEventRequest.java` has no `eventCode` field and `CreateSessionRequest` (events-api.openapi.yml) has no `sessionSlug` field — server-generated for both. Prefix normalization is therefore inapplicable across all 5 collections; `BRUNO-TEST-*` cleanup-endpoint sweep is a no-op for today's fixture events (cleaned up via explicit `98-delete-fixture-event.bru` DELETE by eventCode instead). Filed in §"Risks to track" item 7. |
...
| 8 | _absorbed into PR 6 (batched)_ | ~~D.6: event-topics-api~~ | 🟢 absorbed | — | Step 3 shipped in PR 670. Step 2 already canonical (`bruno-test-topic-{ts}`). Step 4 landed in PR 6 as `67a-verify-event-deleted.bru`. |
| 9 | _absorbed into PR 6 (batched)_ | ~~D.7: events-crud-api~~ | 🟢 absorbed | — | Step 3 shipped in PR 670. Step 2 investigated in PR 6 — `CreateEventRequest` has no `eventCode` field (server generates `BATbern{N}`); filed in §"Risks to track" item 7. Step 4 landed in PR 6 as `29a-verify-event-deleted.bru`. |
| 10 | _absorbed into PR 6 (batched)_ | ~~D.8: sessions-api~~ | 🟢 absorbed | — | Step 3 shipped in PR 670. The 1 pre-existing failure (`21-assign-speaker-to-session.bru` `presentationTitle` assertion) was fixed in PR 670's commit `753c1315`. Step 2 investigated in PR 6 — `CreateSessionRequest` has no `sessionSlug` field; filed in §"Risks to track" item 7. Step 4 landed in PR 6 as `11a-verify-session-deleted.bru`. |
| 11 | `test-enhancement/bruno-audit-batched-closeout` (= PR #671, bundled with PR 6) | D.9: **workflow-aware redesign** per ADR-009 — speaker-pool-api restructure + speaker-portal-api cascade fixes + CUMS USERS regex widening | 🔵 in review (PR #671 → develop) | — | **Speaker-pool-api: 61/78 → 78/78 ✓×2 locally.** Test 38 rewritten as POST /promote (CONTACTED → READY, replacing PUT /status that now returns 400 READY_REQUIRES_PROMOTE_ENDPOINT). New 38a (READY → INVITED). 39 updated (previousStatus=INVITED, not READY). 40/51 switched from message-text assertions to error-code (InvalidStateTransitionException). 41 updated to expect 4 transitions, not 3. 51 swapped invalid enum (CONFIRMED removed by 11.B.1) for valid-enum-invalid-transition (QUALITY_REVIEWED from IDENTIFIED → 422). 47 flipped from 200-idempotent to 409 — the controller's pre-check (`InvalidPromotionStateException`) tightens beyond the workflow service's same-state branch. **Tests 53/54 disabled** with docs (5-speaker setup needed for slot-capacity gate; canonical coverage in `SpeakerInvitationControllerIntegrationTest#should_return409_when_sendInvitationCalledAndSlotCapacityReached`). 55 widened keyword set ("invalid request format" accepted). 45-47 migrated from `promote.e2e.*` to `bruno.test.*` naming for cross-service cleanup. **CUMS USERS regex widened** to `^bruno\.test\.?$` (optional trailing dot) — accepts both `bruno.test` (bare, first-promote collision-free) and `bruno.test.N` (suffixed) via single LIKE `bruno.test%` sweep. New 00a-/99a-pretest/posttest-cleanup-users.bru hooks. **Speaker-portal-api: 4 failures → 37/37 ✓×2 locally.** Tests 02 + 09 fixed via per-speaker promote bridge (01a/01b + 08a/08b — PUT to CONTACTED + POST /promote before /send-invitation). Test 27 fixed via 26b/26c (advance INVITED → ACCEPTED → CONTENT_SUBMITTED). **Test 33 disabled** with docs (speaker-token-bound seed requires 5+ setup .bru files; canonical coverage in `SpeakerResponseControllerIntegrationTest`). |
| 12 | _absorbed into PR 6 (batched)_ | ~~D.10: event-full-workflow-api~~ | 🟢 absorbed | — | Step 3 + state-machine walk + invalid-transition tests shipped in PR 670 / PR 2b. Step 2 inapplicable (same eventCode finding as PR 9; §"Risks to track" item 7). Step 4 landed in PR 6 as `97c-verify-workflow-event-deleted.bru` (conditional on prior DELETE status — 204/404 → assert 404, 409 → event still exists by design, swept later by Story 10.18 archival cleanup). Deferred "registrations CRUD against AGENDA_PUBLISHED" stays out of scope. |
| 13 | `test-enhancement/bruno-audit-partners-api` (= PR #672 → develop) | D.11: light audit + PCS `partner_meetings` entityType extension + **new `partner-meetings-api` collection (written from scratch)** | 🔵 in review (PR #672 → develop); rebased to 1 clean commit; green ×2 locally | — | **All affected collections green ×2 locally: partner-meetings-api 24/24 (15 req), partners-api 63/63 (20 req), admin-cleanup-api 11/11.** PCS Java integration suite green (added 5 MEETINGS tests), Checkstyle clean. **Deliverables:** (1) PCS cleanup `entityType=meetings` via `meetingIds` UUID allowlist (§B2 option 1) — `TestFixtureCleanupService`/`Request`/`Repository`/`Controller`; deletes `partner_meetings` by id, cascading attendance + rsvps (V2/V9); empty/missing/over-cap (200) list → 400; bounded at `MAX_MEETING_IDS=200`. (2) New `partner-meetings-api` Bruno collection — the collection did **not** exist before (CLAUDE.md listed it but no dir on disk); covers create/get/404/list/patch/send-invite(202)/delete/verify-404 + 401/403, with a `98-posttest-cleanup-meetings.bru` that posts the run's captured IDs to the new endpoint (audit log confirmed `entityType=MEETINGS meetingIds=2 counts={meetings=1}`). (3) partners-api audit: added `99b-posttest-cleanup.bru` (canonical `brtest` sweep) + `99-verify-partner-soft-deleted.bru`. (4) admin-cleanup-api `24-pcs-200-meetings` + `25-pcs-400-empty-meetings`. (5) `DELETE /partner-meetings/{id}` added to the OpenAPI spec (was code-only). **Findings:** (a) partners `DELETE` is a **soft delete** (sets `partnershipEndDate`, `isActive=false`; row persists, GET still 200s) — audit-step-4 verify asserts `isActive=false`, not 404. (b) Meetings have no Bruno-identifying column, so there is no `00-pretest` meetings sweep — the allowlist can only reach IDs from the current run (documented trade-off; teardown = explicit DELETE + posttest allowlist sweep). |
| 14 | `test-enhancement/bruno-gate-flip` (= PR #674 → develop) | Section E: remove `continue-on-error`, prove rollback path | 🔵 in review (PR #674 → develop) | — | Workflow changes on `deploy-staging.yml`: removed step-level `continue-on-error` on the Bruno step (failure now fails the deploy); downstream jobs gate on `needs.bruno-tests.result`; `rollback-on-bruno-failure` gained `always()` + dropped the `enable_bruno_rollback` gate (fires on every Bruno failure); `tag-stable-on-success` simplified + `outputs.outcome` removed. **Bonus:** this also **repairs the never-promoted `staging-stable`** ECR tag (the old `outputs.outcome` gate never evaluated true through the continue-on-error step → promote always skipped; confirmed `staging-stable` absent in ECR). First green deploy under the new workflow populates it. **Owed after merge (user-approved sequence):** the F1 deliberate-fail drill (real production-account rollback — low-traffic window). |
...
**Update 2026-06-06 — two more applications of this framing note landed** (branch `fix/bruno-cleanup-topics-notifications`):
- **PCS `entityType=topics`** — `topic_suggestions` (+ `topic_votes` cascade) reference `company_name` as an ADR-003 string, NOT a partner FK, so the `entityType=partners` sweep never reached them and orphans survived aborted runs. New prefix sweep on `title LIKE 'Bruno Test Topic%'` (regex-locked literal `Bruno Test Topic`). Pretest `00-pretest-cleanup-topics.bru` + posttest `99c-posttest-cleanup-topics.bru` in `partners-api/`.
- **EMS `entityType=notifications`** — `notifications` are created as SIDE EFFECTS of entity/state changes (workflow transitions, publishes, registrations) and carry no FK to events; uncleaned, BRUNO-TEST notifications land in REAL organizers' in-app lists on the production account. Composite sweep: `event_code LIKE 'BRUNO-TEST-%'` OR reserved-range `BATbern{N}` (N ≥ 10000) OR `recipient_username LIKE 'bruno.test.%'` OR `subject LIKE '%BRUNO-TEST-%'` OR `body LIKE '%BRUNO-TEST-%'` (regex-locked sentinel `BRUNO-TEST-`). The subject/body markers catch test notifications that carry a **NULL `event_code`** (e.g. a workflow notification whose only test trace is the `BRUNO-TEST-X` sentinel rendered into its text) — rows the event_code/recipient discriminators alone would miss. The reserved-range regex is **bounded to `^BATbern[0-9]{1,9}$`** (≤ 9 digits) so a pathological 10+-digit tail can never overflow the `CAST(... AS INTEGER)` and 500 the whole sweep — such a row simply fails the numeric branch and is left untouched. Pretest + posttest `…-cleanup-notifications.bru` wired into all 6 EMS-writing collections (event-full-workflow-api, events-crud-api, sessions-api, tasks-api, event-topics-api, partner-meetings-api). admin-cleanup-api gained `16/17` (EMS notifications 200/400) and `26/27` (PCS topics 200/400) verification cases.
- **`run_cleanup_for_collection` now glob-discovers cleanup files** — the helper in `scripts/ci/run-bruno-tests.sh` previously hardcoded `("00-pretest-cleanup.bru" "99-posttest-cleanup.bru")`, so `--cleanup-only` mode and the post-failure defensive sweep silently skipped every suffixed cleanup file (the pre-existing `99b-`/`99c-` sweeps AND the new `…-cleanup-topics`/`…-cleanup-notifications` files). It now globs every `*pretest-cleanup*.bru` + `*posttest-cleanup*.bru` in the collection dir and runs them in filename (lexical) order, preserving the non-fatal semantics.
...
| 2 + 2a + 2b + 2c ✅ | `test-enhancement/bruno-events-api-split` (PR 670, 10 commits) | C mechanical-only + event-types-api tests + event-full-workflow state-machine walk + apex-domain CORS hotfix + 405 cross-service handler + dev-DB cleanup SQL + 4 local-dev test fixes. **Squash-merged 2026-05-26 (`57d80f14`); branch deleted.** | warning |
| 3 + 4 + 5 ✅ | `test-enhancement/bruno-audit-{file-upload,companies,users}-api` | D.1 + D.2 + D.3 bundled into #666 with F4 fixes. | warning |
| 6 + 11 🔵 | `test-enhancement/bruno-audit-batched-closeout` (= **PR #671** → develop) | **Bundled PR 6 + PR 11.** PR 6: §D steps 2+4 across tasks-api + 4 PR-670 collections. PR 11: speaker-pool-api ADR-009 redesign + speaker-portal-api cascade fixes + CUMS USERS regex widening + cross-service cleanup hooks. Rebased clean onto squashed develop; all 7 collections green ×2 locally. | warning |
| 7-10 + 12 | _absorbed into PR 6 (batched)_ — see those rows in the Progress log table | | warning |
| 13 🔵 | `test-enhancement/bruno-audit-partners-api` (= **PR #672** → develop) | D.11 light audit + PCS `entityType=meetings` id-allowlist cleanup + brand-new `partner-meetings-api` collection + admin-cleanup-api meetings cases + OpenAPI DELETE. Rebased to 1 clean commit; green ×2 locally. | warning |
| 14 🔵 | `test-enhancement/bruno-gate-flip` (= **PR #674** → develop) | E: removed `continue-on-error`; downstream jobs gate on `needs.bruno-tests.result`; rollback gained `always()` + always-on; also repairs the never-promoted `staging-stable` tag. Owed after merge: F1 deliberate-fail drill. | **blocking + auto-rollback** |
...
| partnerTopicTitle (PCS `entityType=topics`) | `V4__update_topic_tables_for_story_8_2.sql:18` | VARCHAR(255) `topic_suggestions.title` | `Bruno Test Topic - <free text>` (matches `10-suggest-topic-as-partner`); cleanup prefix literal `Bruno Test Topic`, regex `^Bruno Test Topic$`; `topic_votes` cascade via FK | ≤255 chars |
| notificationMarker (EMS `entityType=notifications`) | `V33__Create_notifications_table.sql:9,11` | `event_code` VARCHAR(50) nullable + `recipient_username` VARCHAR(100) + `subject`/`body` | composite sweep: `event_code LIKE 'BRUNO-TEST-%'` OR reserved-range `BATbern{N}` (N ≥ 10000, regex bounded `^BATbern[0-9]{1,9}$` to prevent CAST overflow) OR `recipient_username LIKE 'bruno.test.%'` OR `subject LIKE '%BRUNO-TEST-%'` OR `body LIKE '%BRUNO-TEST-%'` (subject/body markers catch NULL-`event_code` rows); cleanup prefix literal `BRUNO-TEST-`, regex `^BRUNO-TEST-$` | n/a |
...
- Body specifies `entityType` (companies | users | events | sessions | topics | tasks | partners | registrations | speaker_pool | uploads | notifications [EMS, added 2026-06-06] | topics [PCS partner topic_suggestions, added 2026-06-06]) and `prefix`. Note: `topics` is overloaded — EMS `topics` cleans `topics`/`topic_code`, PCS `topics` cleans `topic_suggestions`/`title`; each service binds its own regex.
...
**✅ RESOLVED in PR 13 — partner_meetings cleanup coverage** (discovered 2026-05-25): shipped option 1 below — a `meetingIds` UUID allowlist on the PCS cleanup endpoint (`entityType=meetings`), consumed by the new `partner-meetings-api` collection's `98-posttest-cleanup-meetings.bru`. The original analysis is retained below for context.

PCS owns `partner_meetings` but the table has no FK to `partners` — meetings are standalone events that link to partners only via `partner_meeting_attendance` / `partner_meeting_rsvps` (which cascade from `partners`, not from the meeting itself). The PR 1 PCS cleanup endpoint only handled `entityType=partners`, so:
...
7. **`eventCode` + `sessionSlug` are server-generated, not client-suppliable** (verified 2026-05-26 during PR 6). `CreateEventRequest.java` has no `eventCode` field — server derives `BATbern{eventNumber}`. `CreateSessionRequest` (events-api.openapi.yml) has no `sessionSlug` field — server slugifies from `title`. Consequence: the `00-/99-` cleanup hooks' `entityType=events, prefix=BRUNO-TEST-` sweep is a **no-op for today's fixture events** (they're named `BATbern{N}`); per-run teardown happens via explicit `98-delete-fixture-event.bru` DELETE by eventCode instead. The canonical prefix sweep becomes load-bearing only if the API later accepts client-supplied `eventCode`/`sessionSlug`. Not a bug — a constraint to remember when reasoning about staging cleanup blast radius.
8. **Constraint surprises to verify in PR 1 — these are likely bugs the test work uncovers**:
