...
| 2 | `test-enhancement/bruno-events-api-split` | Section C: decompose events-api into 6 collections **+ bundled apex-domain hotfix from #669 fallout** | 🟡 in progress | — | events-api currently 49/60 on staging — splitting is the unlock for PRs 7-12 audit work. **Bundled hotfix (claude-review on #669, unfixed at merge):** `RateLimitingFilter.java:155` + `TurnstileVerificationFilter.java:199` both ended up with `origin.equals("https://www.batbern.ch") \|\| origin.equals("https://www.batbern.ch")` (apex `https://batbern.ch` dropped from the OR). Plus duplicate `https://www.batbern.ch` entry in `api-gateway/src/test/resources/application-test.yml`. Note: `CorsHandler.ALLOWED_ORIGINS` correctly lists both apex + www — only these two inline filters drifted, because they weren't wired to `CorsHandler.isOriginAllowed`. Decision (2026-05-26): bundle the fix as the first commit on this branch rather than spin a tiny third hotfix PR. |
