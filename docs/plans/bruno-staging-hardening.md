...
> **Where we are (2026-05-27 ~14:30 CET):** **PRs 670, #671, #672, #673, #675 all MERGED** to `develop` (HEAD `479b6b0a`). 🔴 **REAL ROOT CAUSE of "staging keeps breaking after Epic 11" FOUND.** The Bruno gate flip (PR #674) — by removing the `continue-on-error` mask — exposed that **the EventManagement service has been stuck on a stale image (build `1142`, `4cbd315`) since #669**, while every other service is on current build `1158`. EMS's newer images **crash on startup and ECS circuit-breaker-rolls-back to 1142**. Crash cause: **Flyway `Migration checksum mismatch for migration version 86`** — **#669's bulk `cdn.staging.batbern.ch → cdn.batbern.ch` substitution accidentally edited the already-applied `V86__fix_cloudfront_domain_in_media_urls.sql`** (one line: `v_cdn_domain := 'https://cdn.staging.batbern.ch'`), changing its checksum vs the staging DB's `flyway_schema_history`. So all the `/event-types` 404/500 flapping was a *symptom*: staging EMS runs old code and won't boot the new code. **THIS FIX PR (`fix/ems-flyway-v86-checksum`, off `develop`):** (1) **revert V86 byte-exact** to its original content so the checksum matches the staging DB → EMS boots; (2) add **`V105__fix_cdn_staging_domain_in_media_urls.sql`** — the *intended* cdn.staging → cdn.batbern.ch media-URL data fix, done correctly as a forward migration. Validated: `EventTypeControllerIntegrationTest` green (all EMS migrations incl. reverted V86 + V105 apply on a fresh PG; event-types endpoint works). **Two earlier fixes already merged in #675:** the gateway `/api/v1/event-types` route (it genuinely had none) + `rollback-deployment.sh` ECS service-name resolution (it filtered lowercase-hyphen names vs PascalCase CDK fragments → rollback was a no-op). **After this EMS fix lands + EMS finally deploys build 1159+:** `/event-types` works → `event-types-api` green → **rebase + re-attempt PR #674 (gate flip)** → first green deploy promotes the never-set `staging-stable` (PR 14's `tag-stable` fix) → then the F1 drill. **NEW-SESSION PICKUP:** EMS V86/V105 fix is up against develop; once merged + EMS deploys current code, the gate-flip (PR #674) should finally pass. Original PR 13 summary follows:
>
> PR 13 shipped: (a) the new PCS `entityType=meetings` cleanup path — `meeting_id` allowlist (plan §B2 option 1, ~60 LOC + 5 integration tests, PCS suite green), (b) a brand-new `partner-meetings-api` Bruno collection (15 requests, 24/24 ×2), (c) the partners-api light audit (now 20 requests, 63/63 ×2), (d) 2 new admin-cleanup-api meetings auth-matrix cases (11/11 ×2), (e) the missing `DELETE /partner-meetings/{id}` operation added to the OpenAPI spec. **Local Bruno green ×2: partner-meetings-api 24/24, partners-api 63/63, admin-cleanup-api 11/11.** **Findings:** partners `DELETE` is a **soft delete** (sets `partnershipEndDate`, `isActive=false`; row persists) — the audit-step-4 verify asserts `isActive=false` not 404; the canonical hard cleanup is the PCS cleanup endpoint. **Next: PR 14** (gate flip + F1 deliberate-fail exercise) once PR #672 lands and all collections go green ×2 on staging. Out-of-band already landed: **#669** dead-domain retirement (`1a6e2ce2`); **#665/#667/#668** gateway URI double-encoding, CI role-token plumbing, Bruno runner `((var++))` set -e trap. **NEW-SESSION PICKUP:** PR #672 (PR 13) is open against `develop` and awaiting CI/review; after it merges, only PR 14 remains. Nothing else in flight locally.
...
| 14 | `test-enhancement/bruno-gate-flip` | Section E: remove `continue-on-error`, prove rollback path | ⬜ not started — gated on **PR #672 (PR 13) merged + all collections green ×2 on staging** (PR 670 ✅ + #671 ✅ merged) | — | F1 deliberate-fail rollback exercise (workflow_dispatch with `enable_bruno_rollback: true` on a throwaway branch) is owed before the gate flip — should fire once PR #672 lands and all collections go green on staging twice. |
