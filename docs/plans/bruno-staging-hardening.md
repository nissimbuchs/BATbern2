...
> **Where we are (2026-05-27 ~12:00 CET):** **PRs 670, #671, #672 (PR 13), #673 (email hotfix) all MERGED** to `develop` (`293543bd`). Email incident closed. **PR 14 (gate flip) = PR #674, OPEN but BLOCKED.** Flipping Bruno to a blocking gate revealed that the earlier "staging green ×2" was **illusory** — `continue-on-error: true` had been masking a *failing* Bruno step: `event-types-api` has been RED on staging since PR 2a because the **API Gateway has no route for `/api/v1/event-types`** (it doesn't match the `/api/v1/events` prefix → `RoutingException` → 500). #674's own deploy correctly failed on it. The auto-rollback fired (always-on logic ✓) but the **rollback script couldn't find the ECS services** (it filtered for lowercase-hyphen names; ECS uses PascalCase CDK fragments like `EventManagement`, `CompanyManagement`) so it skipped everything — no accidental rollback, but auto-rollback was a no-op. **THIS FIX PR (`fix/gateway-event-types-route-and-rollback-service-names`, off `develop`):** (1) adds `/api/v1/event-types` → `event-management-service` in `DomainRouter` (+ unit test, green); (2) fixes `rollback-deployment.sh` service-name resolution (logical→CDK-fragment map + `resolve_ecs_service_arn` helper across all 3 loops; `--dry-run` now locates all 5 services). **After this lands green:** rebase PR #674, re-deploy — the first green deploy under the flipped gate also **populates the never-promoted `staging-stable`** tag (PR 14 switches `tag-stable` to `needs.bruno-tests.result`), giving rollback a real target. Then the F1 drill. Original PR 13 summary follows:
>
