...
> **Where we are (2026-05-26 ~08:00 CET):** Sections A + B + D (per-entity audit) for the user-facing collections are merged on `develop`. Six PRs landed: **PR 1 (#664)** infra (Sections A + B); **PR 3-5 bundled into #666** (file-upload + companies + users-api audit-passes + CUMS `ADDITIONAL_EMAILS` entityType); **#669** retired the dead `staging.batbern.ch` domain (merged 2026-05-25, commit `1a6e2ce2`) — Bruno post-merge baseline matched prediction (5 passed / 2 failed: file-upload + companies cleared, events-api 49/60 + partners-api 3/18 still red as PR 2 / PR 13 territory). Plus three out-of-band fixes — **#665** gateway URI double-encoding + Cache-Control passthrough; **#667** 11.F.1 magic-link teardown (also fixed the CI role-token plumbing that gated #664's Bruno step on `expected 401 to equal 403`); **#668** Bruno runner counter `((var++))` set -e trap (the runner silently aborted on the first all-green collection, so `tag-stable-on-success` could never fire). **#669 left two apex-domain regressions unfixed** that claude-review caught: `RateLimitingFilter.java:155` + `TurnstileVerificationFilter.java:199` both compare `origin.equals("https://www.batbern.ch") || origin.equals("https://www.batbern.ch")` (the second clause was meant to be the apex `https://batbern.ch`, matching `CorsHandler.ALLOWED_ORIGINS`); `application-test.yml` has duplicate `https://www.batbern.ch` entries. Bundled as the first commit of PR 2 (this branch). Tag-stable promotion **correctly remained SKIPPED** on the #669 run — Bruno still red, gate legitimately closed; #668's counter fix is verified-working on a red-Bruno run. **Next: PR 2** (`test-enhancement/bruno-events-api-split`, Section C) — splits the monolithic 60-test events-api into 6 dependency-ordered collections, with the apex-domain hotfix piggy-backed; PRs 7-12 then do the light audit pass on each. After PR 2 lands, only partners-api stays red — that's PR 13 + the PCS `partner_meetings` cleanup entityType deferred from PR 1. The PR 14 gate flip (`continue-on-error: false` + `enable_bruno_rollback: true`) becomes reachable once events-api and partners-api are green. **F1 deliberate-fail rollback exercise** (workflow_dispatch with `enable_bruno_rollback: true` on a throwaway branch) is still owed before PR 14 — should fire as a one-off after PR 2 lands.
...
| 2 | `test-enhancement/bruno-events-api-split` | Section C mechanical-only: file moves + cleanup hooks for 4 active collections + 2 stubs (event-types-api, event-full-workflow-api gets seeded with tests 12/13/16). Net-new tests deferred to PR 2a + PR 2b. **+ bundled apex-domain hotfix from #669 fallout** | 🟡 in progress | — | events-api was 49/60 on staging pre-split — splitting is the unlock for PRs 7-12 audit work. **Bundled hotfix (claude-review on #669, unfixed at merge):** `RateLimitingFilter.java:155` + `TurnstileVerificationFilter.java:199` both ended up with `origin.equals("https://www.batbern.ch") \|\| origin.equals("https://www.batbern.ch")` (apex `https://batbern.ch` dropped from the OR). Plus duplicate `https://www.batbern.ch` entry in `api-gateway/src/test/resources/application-test.yml`. `CorsHandler.ALLOWED_ORIGINS` correctly lists both apex + www — only these two inline filters drifted because they weren't wired to `CorsHandler.isOriginAllowed`. Fix: both filters now delegate via `corsHandler.isOriginAllowed(origin)` (eliminates the duplicate allow-list); 3 new filter-level apex regression tests in TurnstileVerificationFilterTest. Decision (2026-05-26): bundle the fix as commit 2 on this branch rather than spin a tiny third hotfix PR. |
| 2a | `test-enhancement/bruno-events-api-split` (stacked on PR 2 — GitHub Actions degraded, so stacking instead of separate branch; will rebase post-PR-2-merge) | Net-new event-types-api tests: GET /event-types list + GET specific + GET invalid + idempotent PUT (capture-then-replay, no state drift) + PUT validation 400 + POST/DELETE 404/405 documenting API gap. **+ bundled backend hygiene fix**: HttpRequestMethodNotSupportedException handler in all 4 services' GlobalExceptionHandler (EMS, CUMS, PCS, SCS) — without it, POST/DELETE on a GET-only route returned 500 (same gotcha class as the MethodArgumentNotValidException rule in project-context.md). Discovered while writing test 08/09. PCS gets a unit test (`should_return405_when_methodNotSupportedExceptionThrown`); EMS gets Bruno-level coverage via tests 08/09; CUMS/SCS get the fix without a per-service unit test (handler is dead-simple, identical pattern to PCS). | 🟡 in progress | — | 9 Bruno tests, green ×2 locally (19/19 tests). |
| 2b | `test-enhancement/bruno-events-api-split` (stacked on PR 2/2a — GitHub Actions degraded) | Net-new event-full-workflow-api tests: 7-step state-machine walk CREATED → TOPIC_SELECTION → SPEAKER_IDENTIFICATION → SLOT_ASSIGNMENT → AGENDA_PUBLISHED → EVENT_LIVE → EVENT_COMPLETED → ARCHIVED via PUT `/events/{code}/workflow/transition` with `overrideValidation:true` (business rules owned by sessions-api + speaker-pool-api collections) + 2 invalid-transition rejection tests (CREATED → ARCHIVED and CREATED → EVENT_LIVE without override → 400/422) + 3 fixture events (seed/walk/holdout) + 2 cleanup hooks. Registrations CRUD against `REGISTRATION_OPEN` deferred to a potential PR 2c follow-up — not blocking. | 🟡 in progress | — | 11 net-new Bruno tests, 20/20 requests + 44/44 tests green ×2 locally. |
...
| 6 | `test-enhancement/bruno-audit-tasks-api` | D.4: light audit | ⬜ not started | — | tasks-api already 10/10 on staging — light audit can ride after PR 2 work or before. |
