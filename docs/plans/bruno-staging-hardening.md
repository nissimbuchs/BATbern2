...
> **Where we are (2026-05-26 ~22:00 CET):** Sections A + B + D-partial merged on `develop`. **PR 670 still OPEN** (10 commits awaiting GitHub Actions recovery — major outage since 2026-05-26 ~11:00 UTC). **PR 6 (batched audit-pass closeout) now in review, stacked on PR 670** per explicit user decision (plan's "off `develop` AFTER PR 670 merges" pre-condition relaxed because Actions is unbounded). PR 6 ships: tasks-api canonical hook migration (00/00b/98/99 replacing legacy 00-create-test-event / 99-cleanup-test-event), DELETE-then-GET-404 verify stubs across the 4 PR-670 collections (29a events-crud, 67a event-topics, 11a sessions, 97c event-full-workflow conditional on prior status), and §D step 2 finding (`eventCode` + `sessionSlug` are server-generated — filed as §"Risks to track" item 7, prefix normalization inapplicable). **Local Bruno: all 5 PR-6 collections green ×2** (tasks-api 31/31, events-crud-api 30/30, event-topics-api 26/26, sessions-api 58/58, event-full-workflow-api 46/46). **Next after PR 6 merges: PR 11** (~12 files, ~230 LOC speaker-pool-api redesign per ADR-009), **PR 13** (PCS `partner_meetings` entityType + staging re-verify), **PR 14** (gate flip + F1 deliberate-fail exercise). Out-of-band: **#669** retired the dead `staging.batbern.ch` domain (merged 2026-05-25, commit `1a6e2ce2`); **#665/#667/#668** fixed gateway URI double-encoding, CI role-token plumbing, and the Bruno runner `((var++))` set -e trap respectively.
...
| 6 🔵 | `test-enhancement/bruno-audit-batched-closeout` (stacked on PR 670) | **Batched §D steps 2 + 4 across tasks-api + 4 new collections from PR 2** (events-crud, event-topics, sessions, event-full-workflow; speaker-pool excluded — PR 11). Local green ×2. | warning |
