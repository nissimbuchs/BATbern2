...
> **Where we are (2026-05-26 ~23:30 CET):** Sections A + B + D-partial merged on `develop`. **PR 670 still OPEN** (10 commits awaiting GitHub Actions recovery — major outage since 2026-05-26 ~11:00 UTC). **PR 6 branch now bundles PR 6 + PR 11** (`test-enhancement/bruno-audit-batched-closeout`, stacked on PR 670 per explicit user decision). PR 6 ships the §D step 2/4 closeout across 5 collections; PR 11 ships the Epic 11 / ADR-009 redesign of speaker-pool-api + speaker-portal-api cascade fixes + CUMS USERS regex widening for cross-service cleanup. **Local Bruno: all 7 affected collections green ×2** — tasks-api 31/31, events-crud-api 30/30, event-topics-api 26/26, sessions-api 58/58, event-full-workflow-api 46/46, speaker-pool-api 78/78, speaker-portal-api 37/37. **Next after this branch merges: PR 13** (PCS `partner_meetings` entityType + staging re-verify), **PR 14** (gate flip + F1 deliberate-fail exercise). Out-of-band: **#669** retired the dead `staging.batbern.ch` domain (merged 2026-05-25, commit `1a6e2ce2`); **#665/#667/#668** fixed gateway URI double-encoding, CI role-token plumbing, and the Bruno runner `((var++))` set -e trap respectively.
...
| 6 | `test-enhancement/bruno-audit-batched-closeout` (stacked on PR 670 — Actions outage) | **Batched audit-pass closeout** — Section D steps 2 + 4 across tasks-api + events-crud-api + event-topics-api + sessions-api + event-full-workflow-api. tasks-api gets the canonical hook migration (00/00b/98/99); other four get DELETE-then-GET-404 verify stubs (29a, 67a, 11a, 97c). | 🔵 in review | — | **Local Bruno green ×2 on all 5 collections** (tasks-api 31/31, events-crud-api 30/30, event-topics-api 26/26, sessions-api 58/58, event-full-workflow-api 46/46). **Step 2 finding (verified during impl):** `CreateEventRequest.java` has no `eventCode` field and `CreateSessionRequest` (events-api.openapi.yml) has no `sessionSlug` field — server-generated for both. Prefix normalization is therefore inapplicable across all 5 collections; `BRUNO-TEST-*` cleanup-endpoint sweep is a no-op for today's fixture events (cleaned up via explicit `98-delete-fixture-event.bru` DELETE by eventCode instead). Filed in §"Risks to track" item 7. |
...
| 6 + 11 🔵 | `test-enhancement/bruno-audit-batched-closeout` (stacked on PR 670) | **Bundled PR 6 + PR 11.** PR 6: §D steps 2+4 across tasks-api + 4 PR-670 collections. PR 11: speaker-pool-api ADR-009 redesign + speaker-portal-api cascade fixes + CUMS USERS regex widening + cross-service cleanup hooks. All 7 collections green ×2 locally. | warning |
