...
**Phase 4b — Tone.js out of the eager vendor chunk (added 2026-06-01).** The homepage console always logged *"The AudioContext was not allowed to start…"* (×2). Traced (LayoutShift-style observer + `AudioContext` constructor hook in the preview) to `standardized-audio-context`'s capability probes running at module-init inside the **eager `vendor` chunk**. Root cause: the blanket `manualChunks → 'vendor'` rule pulled Tone.js + `standardized-audio-context` + `automation-events` into `vendor`, overriding the careful dynamic `import('tone')` in `useBlobSounds` (which is otherwise type-only + gesture-gated). Fix: in `vite.config.ts` `manualChunks`, return `undefined` for those three audio modules so they stay in the lazy `import('tone')` chunk (loaded only on the organizer blob page, after a click). Shared `tslib`/`@babel/runtime` deliberately left in `vendor`. Verified on the preview: audio chunk no longer loads on the homepage, **0 AudioContext warnings**, console clean.

**Intentionally NOT done:** splitting the rest of the single `~1.84 MB vendor` chunk. `vite.config.ts:216-232` documents that one-vendor-chunk is deliberate — finer splitting reintroduces the `@emotion`/`@mui` CJS→ESM factory-boundary init-order crash (TDZ / "Cannot set properties of undefined"). Against the auto-rollback staging gate that gamble isn't worth a few KB. The remaining homepage JS cost (`vendor` 573 KB gzip + `vendor-mui` 157 KB, both loaded because the root wraps everything in MUI `ThemeProvider`) would require separating the public app from the authed app — a larger refactor out of scope here. Skipped `rollup-plugin-visualizer` to avoid a lockfile change; sizes read from build output.
