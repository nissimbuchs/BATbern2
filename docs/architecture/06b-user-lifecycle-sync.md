...
- **language** — from the `language` field of `custom:preferences` (`language || "de"`, exactly like `post-confirmation.ts`). When absent/malformed, the row falls back to the embeddable default `"de"` (`UserPreferences`). _This closed the last `custom:preferences` divergence vs. PostConfirmation (Story 12.3); before it, a Swiss-French / EN signup reaching JIT silently lost its chosen language._
