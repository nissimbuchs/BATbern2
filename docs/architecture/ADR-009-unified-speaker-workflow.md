...
| `magic_link_tokens` table[^magic-link-tokens] | Magic-link auth replaced by Cognito FORCE_CHANGE_PASSWORD flow. Dropped by Flyway V104 in Story 11.F.1. |

[^magic-link-tokens]: The colloquial label `magic_link_tokens` refers to the actual EMS table `speaker_invitation_tokens` (created in `services/event-management-service/src/main/resources/db/migration/V43__Create_speaker_invitation_tokens.sql`, dropped in `V104__drop_speaker_invitation_tokens_table.sql`). The default schema-ownership note at the head of this table — "all in the event-management-service schema unless noted" — applies here without override. Resolves the schema-ownership clarification asked in `_bmad-output/implementation-artifacts/deferred-work.md` line 14.

_Implemented by Stories 11.B.3 (Flyway V93), 11.C.1 (Flyway V94), 11.F.1 (Flyway V104)._
...
| 2026-05-25 | 1.7 | Story 11.F.1 magic-link teardown — Phase F (final phase of Epic 11). Deleted `MagicLinkService`, `JwtConfig` (speaker), `SpeakerMagicLoginController`, `SpeakerPortalTokenController`, `E2ETestTokenController`, the four magic-link DTOs (`SpeakerMagicLoginRequest`, `SpeakerAuthResponse`, `ValidateTokenRequest`, `TokenValidationResult`), `SpeakerInvitationToken` entity, `SpeakerInvitationTokenRepository`, `TokenActionConverter`, shared-kernel `TokenAction` enum, `TokenCleanupScheduler`, `InvalidTokenException`, frontend `SpeakerMagicLoginPage.tsx` + `speakerAuthService.ts`, ~19 magic-link Bruno tests, 5 `@Disabled` legacy speaker-portal integration test classes, 4 `describe.skip` Vitest suites, and `scripts/e2e/generate-speaker-tokens.sh`. Flyway V104 drops the `speaker_invitation_tokens` table (the colloquial `magic_link_tokens` label in §0.3 — see footnote). New `MagicLinkEndpointsRemovedIntegrationTest` + two Bruno 404 regression tests lock in the teardown. New `GlobalExceptionHandler` branch translates `NoHandlerFoundException` / `NoResourceFoundException` to 404 (Spring 6.x adapter). Four new Cognito-side Vitest suites (`ContentSubmissionPage.test.tsx`, `InvitationResponsePage.test.tsx`, `ProfileUpdatePage.test.tsx`, `speakerPortalService.test.ts`) replace the deleted magic-link tests per RD5. `viewToken` + `portalToken` parameters dropped from `SpeakerAcceptanceEmailService` / `SpeakerReminderEmailService`; the dashboard / content / profile links in acceptance + reminder emails point at Cognito-secured routes (no `?token=` query parameter). | Amelia (Dev Agent) |
