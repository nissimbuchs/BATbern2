...
- **ADR-010**: Federated Identity via Cognito (Google SSO over OIDC) — Cognito as OIDC broker (backend JWT contract unchanged), existing accounts link via `AdminLinkProviderForUser` in `PreSignUp_ExternalProvider`, new Google users auto-provision as ATTENDEE. Federation forces provisioning onto canonical JIT (PostConfirmation removed) and inactive-blocking onto a new API-Gateway `is_active` gate (PreAuthentication retired); `companyId` removed from the token (resolved via user-api). Target trigger set: `PreSignUp`, `PreTokenGeneration`, `CustomEmailSender`. Google-only first; Apple/OIDC deferred. See `ADR-010-federated-identity-via-cognito.md` and the plan in `docs/plans/sso-oidc-federation.md`.
- **ADR-011**: Public Site Static Extraction — CloudFront Routing & Island Auth-Bootstrap. Resolves the source plan's two open design questions for Epic 13. **Routing:** one bucket / one distribution; **extend the existing `routerFunction` CloudFront Function** (the repo already rewrites `/`,`/about`,`/privacy`,`/support` to baked HTML) with an `/archive/:code` prefix rule — reject two origins. Two `dist/` trees (Astro + SPA) merge into one bucket with disjoint keys; the `404→/index.html` net makes every step fail-safe and lets the Function ship ahead of content. **Auth islands:** same-origin Cognito session sharing means islands read the SPA session for free; one interactive-shell React root (nav + hero registration) owns the minimal provider subset (no `AuthContext` refactor); public-only widgets (slots/capacity, countdown, newsletter, archive filter) are separate lazy islands with no auth providers. Anonymous visitors/crawlers load no Amplify (`hasCognitoSession()` + lazy-Amplify gate). See `ADR-011-public-site-static-architecture.md`, the plan in `docs/plans/public-site-static-extraction.md`, and Epic 13 in `docs/prd/epic-13-public-site-static-extraction.md`.
