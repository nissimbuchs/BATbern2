...
- [ ] Task 1: AWS Cognito Configuration (AC: 1,2,4)
  - [ ] Write failing tests for Cognito user pool creation
  - [ ] Write failing tests for custom attribute configuration
  - [ ] Write failing tests for role-based user registration
  - [ ] Implement Cognito CDK stack with user pools and custom attributes
  - [ ] Configure MFA settings for organizer and partner roles
  - [ ] Verify all Cognito tests pass

- [ ] Task 2: JWT Token Validation (RED Phase) (AC: 3)
  - [ ] Write failing tests for JWT token validation logic
  - [ ] Write failing tests for token expiration handling
  - [ ] Write failing tests for user context extraction
  - [ ] Write failing tests for token refresh scenarios
  - [ ] Verify all tests fail with meaningful error messages

- [ ] Task 3: JWT Token Implementation (GREEN Phase) (AC: 3)
  - [ ] Implement CognitoJWTValidator for token validation
  - [ ] Implement UserContextExtractor for claims processing
