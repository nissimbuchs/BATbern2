...
- [ ] Task 1: Project Setup & Configuration (Infrastructure Foundation)
  - [ ] Initialize Vite React TypeScript project in `web-frontend/` directory
  - [ ] Configure `tsconfig.json` with strict TypeScript settings
  - [ ] Install core dependencies (React 18.2+, Material-UI 5.14+, Zustand 4.4+, React Query 4.36+, React Router v6)
  - [ ] Install dev dependencies (Vitest 1.0+, React Testing Library 13.4+, Playwright 1.40+, ESLint, Prettier)
  - [ ] Configure Vite build settings (`vite.config.ts`) with code splitting and optimization
  - [ ] Create environment variable files (`.env.development`, `.env.staging`, `.env.production`)
  - [ ] Configure Material-UI theme with Swiss design standards
  - [ ] Set up Tailwind CSS 3.3+ for utility-first styling

- [ ] Task 2a: i18n Infrastructure TDD (RED Phase) (AC: 4, 9)
  - [ ] Write failing tests for i18n configuration and language detection
  - [ ] Write failing tests for translation key type safety and namespace loading
  - [ ] Write failing tests for language switcher component (German/English toggle)
  - [ ] Write failing tests for date/time formatting with date-fns locales

- [ ] Task 2b: i18n Infrastructure Implementation (GREEN Phase) (AC: 4, 9)
  - [ ] Configure react-i18next with i18next-browser-languagedetector
  - [ ] Create translation JSON files in `public/locales/de/` and `public/locales/en/`
  - [ ] Generate TypeScript types from translation files for type safety
  - [ ] Implement language detection priority (User Profile → LocalStorage → Browser → Default)
  - [ ] Create date/time formatting utils using date-fns with locale support
  - [ ] Implement LanguageSwitcher component with API integration

- [ ] Task 3a: Authentication Integration TDD (RED Phase) (AC: 5)
  - [ ] Write failing tests for AWS Cognito JWT token storage and retrieval
  - [ ] Write failing tests for automatic token refresh logic
  - [ ] Write failing tests for logout and session cleanup

- [ ] Task 3b: Authentication Integration Implementation (GREEN Phase) (AC: 5)
  - [ ] Configure AWS Amplify for Cognito integration
  - [ ] Implement `useAuth` hook with token management and auto-refresh
  - [ ] Create auth context provider wrapping application root
  - [ ] Implement axios interceptor for adding `Authorization: Bearer {token}` header
  - [ ] Implement logout functionality with token revocation and cache clearing

- [ ] Task 4a: API Client Layer TDD (RED Phase) (AC: 6)
  - [ ] Write failing tests for type-safe API client methods
  - [ ] Write failing tests for error handling (4xx, 5xx responses)
  - [ ] Write failing tests for request/response interceptors

- [ ] Task 4b: API Client Layer Implementation (GREEN Phase) (AC: 6)
  - [ ] Create base axios API client with environment-based base URL
  - [ ] Implement request interceptor for auth headers and correlation IDs
...
- [ ] Task 5a: State Management TDD (RED Phase) (AC: 7)
  - [ ] Write failing tests for Zustand auth store (user, role, login, logout actions)
  - [ ] Write failing tests for Zustand UI store (locale, sidebar, notifications)
  - [ ] Write failing tests for React Query user profile query and caching
  - [ ] Write failing tests for React Query notifications query and manual refresh

- [ ] Task 5b: State Management Implementation (GREEN Phase) (AC: 7)
  - [ ] Create Zustand auth store with user and role management
  - [ ] Create Zustand UI store with locale and UI state
  - [ ] Configure React Query provider with optimized cache settings
  - [ ] Implement `useUserProfile` hook with React Query
  - [ ] Implement `useNotifications` hook with React Query (manual reload only)
  - [ ] Implement state persistence to localStorage for auth and UI state
...
- [ ] Task 10b: Routing & Route Guards Implementation (GREEN Phase) (AC: 2)
  ...
  - [ ] Implement ProtectedRoute wrapper component with role validation
  - [ ] Define role-specific routes (organizer, speaker, partner, attendee dashboards)
  - [ ] Implement redirect logic for unauthorized access (403 → dashboard)
  - [ ] Implement redirect logic for session expired (401 → login with returnUrl)
