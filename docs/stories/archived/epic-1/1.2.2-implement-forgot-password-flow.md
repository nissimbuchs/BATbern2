...
### Task 1a: Custom Message Lambda Trigger (Infrastructure - FIX MISSING EMAIL TEMPLATES)
**CRITICAL: This task fixes the issue where Cognito sends plain verification codes instead of branded emails**

- [x] Create Lambda function: `infrastructure/lib/lambda/triggers/custom-message.ts`
- [x] Implement `CustomMessage_ForgotPassword` event handler
- [x] Detect user language from `custom:language` attribute (fallback to 'en')
- [x] Build reset link: `https://{frontendDomain}/auth/reset-password?code={code}&email={email}&lang={lang}`
- [x] Call SES to send email with template `BATbern-{envName}-PasswordReset-{DE|EN}`
- [x] Pass `resetLink` template variable to SES
- [x] Add error handling with CloudWatch logging (non-blocking, allows Cognito fallback)
- [x] Return custom email message to Cognito (suppress Cognito's default email by returning empty response)
- [x] Wire Lambda trigger to Cognito User Pool in `cognito-stack.ts`
- [x] Update IAM permissions for Lambda to send SES emails (`ses:SendTemplatedEmail`, `ses:SendEmail`)
- [x] Add environment variables: `FRONTEND_DOMAIN`, `SES_TEMPLATE_PREFIX`, `FROM_EMAIL`
- [x] Write unit tests: `infrastructure/lib/lambda/triggers/__tests__/custom-message.test.ts` (12 test scenarios)
- [x] Test Lambda with mock Cognito event (German/English, error handling, trigger filtering)
- [ ] Deploy and verify branded emails are sent with clickable reset links
...
### SOLUTION: Custom Message Lambda Trigger (Task 1a)

**CRITICAL ISSUE IDENTIFIED (2025-10-24):**
- Cognito is sending **plain verification codes** ("The verification code to your new account is 258936")
- SES templates in `ses-stack.ts` exist but are **NEVER used** by Cognito
- Root cause: No `CustomMessage` Lambda trigger configured in `cognito-stack.ts`

**SOLUTION APPROACH:**
Use AWS Cognito's `CustomMessage` Lambda trigger to intercept `ForgotPassword` events and send custom SES emails with branded templates and clickable reset links.

**Implementation Pattern:**
```typescript
// infrastructure/lib/lambda/triggers/custom-message.ts
...
export const handler = async (event: CustomMessageTriggerEvent) => {
  ...
  if (event.triggerSource === 'CustomMessage_ForgotPassword') {
    const { email, userAttributes, codeParameter } = event.request;
    const language = userAttributes['custom:language'] || 'en';
    const code = codeParameter;

    // Build reset link
    const resetLink = `${process.env.FRONTEND_DOMAIN}/auth/reset-password?code=${code}&email=${encodeURIComponent(email)}&lang=${language}`;

    // Send SES email with template
    const sesClient = new SESClient({});
    const templateName = `${process.env.SES_TEMPLATE_PREFIX}-PasswordReset-${language.toUpperCase()}`;
...
**Wire to Cognito Stack:**
```typescript
// infrastructure/lib/stacks/cognito-stack.ts (add to user pool configuration)
const customMessageTrigger = new lambda.Function(this, 'CustomMessageTrigger', {
  runtime: lambda.Runtime.NODEJS_20_X,
  handler: 'custom-message.handler',
  code: lambda.Code.fromAsset('lib/lambda/triggers'),
  environment: {
    FRONTEND_DOMAIN: props.frontendDomain,
    SES_TEMPLATE_PREFIX: `BATbern-${props.envName}`,
    FROM_EMAIL: 'noreply@berner-architekten-treffen.ch',
  },
});

// Grant SES permissions
customMessageTrigger.addToRolePolicy(new iam.PolicyStatement({
  actions: ['ses:SendTemplatedEmail'],
  resources: ['*'],
}));

// Wire to user pool
userPool.addTrigger(cognito.UserPoolOperation.CUSTOM_MESSAGE, customMessageTrigger);
```

**Benefits:**
- ✅ Uses existing SES templates (no duplicate template management)
- ✅ Clickable reset links (better UX than typing codes)
- ✅ Bilingual support (reads `custom:language` attribute)
- ✅ Branded emails with HTML styling
- ✅ Suppresses Cognito's plain text default email
- ✅ Follows existing Lambda trigger pattern (PostConfirmation, PreTokenGeneration)

### ❌ DEPRECATED: Cognito User Pool Email Template Configuration

**This approach is OUTDATED - Cognito's built-in templates don't support HTML or clickable links.**
...
- [x] **Infrastructure (Task 1a):** `infrastructure/lib/lambda/triggers/custom-message.ts` - Custom Message Lambda trigger (258 lines, i18n support)
- [x] **Infrastructure (Task 1a):** `infrastructure/lib/lambda/triggers/__tests__/custom-message.test.ts` - Unit tests for Custom Message Lambda (12 test scenarios)
...
- [x] **Infrastructure (Task 1a):** `infrastructure/lib/stacks/cognito-stack.ts` - Added CustomMessage Lambda trigger with SES/CloudWatch IAM permissions
