    ...
    mutationFn: async (data: RegistrationFormData) => {
      const command = new SignUpCommand({
        ...
        UserAttributes: [
          ...
          { Name: "custom:language", Value: i18n.language },
          { Name: "custom:newsletter_optin", Value: data.newsletterOptIn.toString() },
        ],
      });

      const response = await cognitoClient.send(command);

      // PostConfirmation Lambda will create database record automatically
