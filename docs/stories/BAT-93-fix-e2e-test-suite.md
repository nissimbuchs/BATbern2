...
### Change Log
- 2026-01-18: BAT-97 IN PROGRESS - Auth tests (forgot-password, reset-password) helper functions updated with language-independent selectors. 2 commits: added data-testid to ForgotPasswordForm (forgot-password-title, forgot-password-submit, back-to-login-link, retry-button) and ResetPasswordForm (reset-password-title, reset-password-submit). Helper functions now use data-testid instead of text matching. Several tests still fail due to missing Cognito/MailHog integration in local dev. Commits: 94820a40, c1e3df8e
