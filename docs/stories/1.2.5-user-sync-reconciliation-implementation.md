...
7. **Bootstrap organizer creation works for all environments**
   ...
   - And the user is created with permanent password `***REMOVED***` (not temporary)
...
- [ ] Task 1: Write Lambda Trigger E2E Tests (RED Phase)
  - [ ] Write failing E2E test for user registration → DB creation flow
  - [ ] Write failing E2E test for JWT token enrichment with DB roles
  - [ ] Verify tests fail with meaningful errors

- [ ] Task 2a: PostConfirmation Lambda TDD Tests (RED Phase) (AC: 1)
  - [ ] Write failing unit tests for user creation logic
  - [ ] Write failing unit tests for role assignment logic
  - [ ] Write failing unit tests for idempotency (ON CONFLICT)
  - [ ] Write failing unit tests for error handling (DB unavailable)
  - [ ] Write failing integration test with PostgreSQL container

- [ ] Task 2b: PostConfirmation Lambda TDD Implementation (GREEN Phase) (AC: 1)
  - [ ] Implement PostConfirmation handler to make tests pass
  - [ ] Implement database connection pooling
  - [ ] Implement ON CONFLICT upsert logic
  - [ ] Implement error logging without blocking Cognito

- [ ] Task 3a: PreTokenGeneration Lambda TDD Tests (RED Phase) (AC: 1)
  - [ ] Write failing unit tests for role fetching from DB
  - [ ] Write failing unit tests for JWT claim enrichment
  - [ ] Write failing unit tests for fallback on DB error
  - [ ] Write failing integration test with PostgreSQL container

- [ ] Task 3b: PreTokenGeneration Lambda TDD Implementation (GREEN Phase) (AC: 1)
  - [ ] Implement PreTokenGeneration handler to make tests pass
  - [ ] Implement role query with event-specific roles
  - [ ] Implement JWT claims override logic
...
- [ ] Task 9a: Bootstrap Organizer TDD Tests (RED Phase) (AC: 7)
  - [ ] Write failing unit tests for bootstrap user creation
  - [ ] Write failing unit tests for permanent password assignment
  - [ ] Write failing integration test with CDK stack deployment

- [ ] Task 9b: Bootstrap Organizer TDD Implementation (GREEN Phase) (AC: 7)
  - [ ] Create CDK construct for bootstrap user creation
  - [ ] Implement AdminCreateUser with permanent password (MessageAction: SUPPRESS)
  - [ ] Configure user: `nissim@buchs.be`, password: `***REMOVED***`
  - [ ] Set email_verified to true to skip verification
  - [ ] Implement idempotency (check if user exists first)
  - [ ] Add custom attribute `custom:batbern_role = ORGANIZER`
...
2. **Deploy Auth Stack First:**
   ...
   cdk deploy BATbernAuthStack-dev --context bootstrap-organizer='{"email":"nissim@buchs.be","password":"***REMOVED***","role":"ORGANIZER"}'
