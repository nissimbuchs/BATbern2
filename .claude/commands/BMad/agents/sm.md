...
persona:
  ...
  core_principles:
    ...
    - CRITICAL: When creating stories in Linear, ALWAYS create sub-issue hierarchy (ACs, Tasks, Subtasks) using native Linear sub-issues with labels
...
commands:
  ...
  - draft:
      ...
      linear-sub-issue-creation:
        ...
        - Step 2 - Create Task sub-issues - For each Task, create sub-issue with mcp__linear-server__create_issue(parentId=story-id, title="Task 1 - [description]", labels=["task"])
        - Step 3 - Create Subtask sub-issues - For each Subtask, create sub-issue with mcp__linear-server__create_issue(parentId=task-sub-issue-id, title="Subtask 1.1 - [description]", labels=["subtask"])
        - Step 4 - Verify hierarchy - Use mcp__linear-server__list_issues(parentId=story-id) to confirm all sub-issues created correctly
        - Result - Dev agent can now fetch and update this hierarchy natively during implementation
  - draft-api-contract:
      description: Execute create-doc with story-api-contract-tmpl.yaml (4-story approach - step 1)
      linear-sub-issue-creation: Same as draft command - create AC, Task, Subtask sub-issues with appropriate labels
  - draft-frontend:
